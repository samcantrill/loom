...
def test_slurm_planned_submission_manifest_schema_is_stable_plain_data() -> None:
    ...
    assert '"argv":["historical-worker","run-1"]' in rendered
...
def test_slurm_command_metadata_round_trips_in_manifest_plain_data() -> None:
    ...
    manifest = SlurmPlannedSubmission(
        ...
        jobs=(
            SlurmPlannedJob(
                logical_key="pipeline", mode=SlurmMode.SINGLE_JOB, command=command
            ),
        ),
        generated_command_argv=(command,),
    )
...
def test_slurm_live_manifest_schema_extends_canonical_manifest_path() -> None:
    ...
    assert (
        SlurmLiveSubmissionManifest.from_dict(payload).to_dict() == manifest.to_dict()
    )
            ...
            **manifest.to_dict(),
            ...
            "cancellation_attempts": [
                SlurmCancellationAttempt(
                    ...
                    message="cancelled",
                    command_record=SlurmCommandResult(
                        command="scancel", argv=("scancel", "123456"), returncode=0
                    ),
                ).to_dict()
            ],
        }
...
def test_slurm_live_manifest_rejects_dependency_ids_without_upstream_submission() -> (
    None
):
    command = SlurmCommandArgv(
        launcher_argv=("historical-worker",), command_args=("run-1",)
    ...
    payload = {
        ...
        "jobs": [
            SlurmPlannedJob(
                logical_key="stage:build", mode=SlurmMode.AFTEROK, command=command
            ).to_dict(),
            SlurmPlannedJob(
        ...
        "dependencies": [
            SlurmPlannedDependency(
                job_key="stage:train", upstream_job_keys=("stage:build",)
            ).to_dict()
        ],
