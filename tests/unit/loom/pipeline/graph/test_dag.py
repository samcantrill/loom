...
from loom.pipeline.errors import PipelineCycleError, PipelineGraphError, PipelineSpecError
from loom.pipeline.graph import StageEdgeReason, StageEdge, build_stage_graph
...
def test_build_stage_graph_adds_data_and_control_edges() -> None:
    ...
    assert any(edge == StageEdge("a", "b", StageEdgeReason.CONTROL) for edge in graph.edges)
    assert any(
        edge == StageEdge("b", "c", StageEdgeReason.DATA, input_name="in", output_name="value")
