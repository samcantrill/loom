...
from loom.plugins.entrypoints import LOOM_CODECS_GROUP, PluginInvalidEntryPointError, PluginRecord
from loom.pipeline.execution.models import RunRequest
from loom.pipeline.execution.errors import RunRequestError


def _record(name: str = "example", value: str = "project.plugins:codec") -> PluginRecord:
    return PluginRecord(group=LOOM_CODECS_GROUP, name=name, value=value, package="project", package_version="1")
...
def test_activation_manifest_round_trips_strict_sorted_identity() -> None:
    ...
    assert [item["name"] for item in manifest.to_dict()["plugins"]] == ["a", "z"]
...
def test_selection_is_exact_applicable_and_does_not_accept_duplicates() -> None:
    record = _record()
    assert resolve_plugin_selections(("loom.codecs:example",), (record,), allowed_groups=(LOOM_CODECS_GROUP,)) == (record,)
    with pytest.raises(PluginInvalidEntryPointError):
        parse_plugin_selector("loom.codecs:example:extra")
    with pytest.raises(PluginInvalidEntryPointError):
        resolve_plugin_selections(("loom.executors:example",), (record,), allowed_groups=(LOOM_CODECS_GROUP,))


def test_reconstruction_comparison_never_invents_distribution_identity() -> None:
    recorded = PluginRecord(group=LOOM_CODECS_GROUP, name="example", value="project.plugins:codec")
...
def test_caller_metadata_cannot_claim_plugin_activation_authority() -> None:
    ...
    assert compare_plugin_activation_records((_record(),), (_record(value="project.plugins:other"),)) == (
