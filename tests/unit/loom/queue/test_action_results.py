...
def _select(store, run="run-a", node="generate", *, principal="one", key="a"):
    return store.select(run_uri=run, node=node,
                        scope={"principal": principal}, identity={"digest": key * 64})


def _publish(store, claim):
    store.attach_fence(claim, run_uri="run-a", node="generate",
                       attempt_id="attempt-a", assignment_id="assignment-a", fencing_token="fence-a")
    result = {"commit": "original-commit", "artifact": "original-artifact"}
    store.publish(claim, result=result, attempt_id="attempt-a",
                  assignment_id="assignment-a", fencing_token="fence-a")
    return result
...
def test_binding_keeps_original_result_and_rejects_cancel_during_verification(tmp_path):
    ...
    binding = store.bind(run_uri="run-b", node="generate", claim_id=claim,
                         expected_revision=candidate["revision"], result=result,
    ...
    with pytest.raises(QueueConflictError, match="no longer live"):
        store.bind(run_uri="run-c", node="generate", claim_id=claim,
                   expected_revision=candidate["revision"], result=result,
                   verification={"verdict": "verified"})
...
def test_stale_fence_cannot_publish_and_failed_producer_is_not_a_miss(tmp_path):
    ...
    with pytest.raises(QueueConflictError, match="fence conflicts"):
        store.publish(claim, result={"commit": "stale"}, attempt_id="attempt-a",
                      assignment_id="assignment-a", fencing_token="stale")
