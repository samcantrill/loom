...
def test_nested_recipe_expansion_path_and_manifest(tmp_path: Path) -> None:
    ...
    assert composed.resolved["pipeline"]["outer"]["value"] == "root"
    assert composed.resolved["pipeline"]["outer"]["inner"]["value"] == "seeded:root-inner"
...
def test_recipe_output_final_interpolation(tmp_path: Path) -> None:
    ...
    assert composed.resolved["pipeline"]["resolved"] == "root-resolved"
    assert composed.resolved["pipeline"]["nested"]["value"] == "nested:root-child"
