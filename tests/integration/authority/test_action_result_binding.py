...
from loom.pipeline.stores.authority import CancellationEpochRequest, CoordinatorAdmissionRequest
from loom.pipeline.stores.read_models import ActionResultBinding, AuthoritativeRunSnapshot
...
def _producer(tmp_path):
    ...
    attempt = store.allocate_stage_attempt(uri, "original", owner_id="worker", lease_ttl_seconds=300)
    committed = store.record_output_commit(uri, "original", attempt_id=attempt.attempt.attempt_id,
                                          fencing_token=attempt.lease.fencing_token,
                                          outputs={"out": ArtifactRef(artifact_id="original/out", uri=uri + "/out", artifact_type="text")})
    binding = ActionResultBinding(claim_id="claim-a", execution_key="sha256:" + "a" * 64,
                                  commit=committed.commit, artifact_facts=committed.artifact_facts,
                                  verification={"schema_version": 1, "candidate_digest": "sha256:" + "b" * 64, "verdict": "verified"})
...
def _consumer(tmp_path, backend):
    ...
    authority.bind_coordinator_admission(uri, CoordinatorAdmissionRequest(
        operation_id="admit-consumer", coordinator_id="coordinator", run_uri=uri, intent_digest="intent"))
    return uri, authority, database, tables


@pytest.mark.parametrize("backend", ["embedded", "authenticated"])
def test_binding_projects_original_commit_without_new_attempt_or_commit(tmp_path, backend):
    ...
    revision = authority.bind_action_result(uri, "renamed-consumer", binding,
    ...
    with pytest.raises(ValueError, match="binding conflicts"):
        authority.bind_action_result(uri, "renamed-consumer", replace(binding, claim_id="different"))
...
def test_binding_cannot_cross_cancellation_barrier(tmp_path, backend):
    ...
    authority.install_cancellation_epoch(uri, CancellationEpochRequest(
        operation_id="cancel-consumer", coordinator_id="coordinator", run_uri=uri,
