...
- Refine pass: pending for the assigned expanded path.
- Phase implementation refinement budget: unused.
- Pre-submit blocker gate budget: unused; before PR submission, the manager must review the phase plan, diff, PR body draft, validation evidence, scope boundary, and known review risks, and must resolve concrete blockers before submitting or mark the phase blocked.
- PR review budget: unused; the pre-submit blocker gate may consume it if it reviews the implementation diff, PR body, and suite evidence before submission.
...
- Existing files or modules that constrain this phase: `src/loom/config/compose.py` owns the shared public staged orchestration and currently emits `artifact_placeholders` with empty `source_artifacts` and `fingerprint_records`; `src/loom/config/api.py` owns `ComposedConfig` and `ConfigCompositionInspection`; `src/loom/config/artifacts.py` owns `CompositionManifest`, `SourceArtifactRecord`, and `ConfigFingerprintRecord`; `src/loom/config/provenance.py` owns `ConfigProvenance`, `ConfigSource`, and `ParsedOverride`; `src/loom/config/includes.py` exposes include-site, local-customization, and recomposition records; `src/loom/config/interpolation.py` exposes resolver scan records; `src/loom/config/recipes/manifest.py` emits recipe manifest records; `src/loom/config/redaction.py` owns key-pattern redaction.
...
- Populate `ComposedConfig.source_artifacts` and `inspection.source_artifacts` with default metadata/hash `SourceArtifactRecord` values for base config, overlays, included files, and recipe source references where the current recipe catalog/manifest exposes stable metadata.
...
- Add warnings/docs snippets for plaintext secrets in override strings, recommending environment/runtime secret mechanisms and supported resolver references instead of literal `+auth.token=...` style values.
- Keep `compose_config(...)` and `inspect_config_composition(...)` using the same staged path and update the stage names or payloads additively. If replacing `artifact_placeholders`, preserve reviewable stage semantics and update contract tests deliberately.
...
- Source artifact records may include absolute paths as provenance context, but semantic references should prefer stable source role/order, config path, content digest, and artifact IDs or equivalent metadata when available.
- Include files currently travel through include records and loader `ConfigSource` metadata; the source artifact layer may label them as `include` even if lower-level loading reused an overlay-style source kind internally.
- Recipe source records are best-effort in this phase: record stable recipe target/catalog metadata already present in recipe manifests, and add a `recipe` source artifact only when it can be represented without introspecting raw project source bytes or adding project-code dependencies.
...
Source records and manifest references may record metadata, content hashes, roles, source order, source paths, include paths, authored include strings, recipe targets, resolver-expression paths, redaction/security policy facts, and stable plain-data summaries. They must not contain raw source bytes, resolved environment values, custom resolver outputs, constructed runtime objects, or the fully resolved runtime config.
...
- Expected PR size and shape: contract-heavy config artifact population under `loom.config`, targeted docs snippet for secret overrides, and package/unit/contract/integration tests. No pipeline, store, CLI, raw snapshot, resume comparison, or final fingerprint algorithm diff.
...
2. Add source artifact population from existing source/load/include/recipe metadata, with stable source role/order and content digest records for base, overlays, includes, and safe recipe references.
...
4. Update redacted artifact population so default redaction applies to unresolved artifact-safe config before artifact serialization, with resolver expressions preserved and sensitive authored paths redacted.
5. Enrich provenance and inspection stage payloads additively so compose and inspection expose consistent artifact-safe records without duplicate composition.
...
- Required assertions or deferral reason: public config exports remain source-compatible; any additive artifact/provenance exports are intentional and cheap; importing `loom.config` does not import pipeline/store/CLI modules; `loom.pipeline` remains independent from `loom.config`, `ComposedConfig`, and manifests.

### Unit Suite

- Status: required.
- Expected paths: `tests/unit/loom/config/test_config_artifacts.py`, `tests/unit/loom/config/test_config_provenance.py`, `tests/unit/loom/config/test_redaction.py`, `tests/unit/loom/config/test_compose.py`, and focused additions near include/recipe/interpolation tests only if those helpers change.
- Required assertions or deferral reason: source artifact records are populated for base/overlay/include sources with metadata/hash-only payloads; manifest source references match populated source artifacts; provenance records include artifact-safe include/override/recipe/resolver/security facts; redaction handles secret-like keys and paths in unresolved data without mutating inputs; compose returns no placeholder-only manifest/source state; resolved runtime values remain out of artifact fields.
...
- Required assertions or deferral reason: `CompositionManifest`, `SourceArtifactRecord`, and `ConfigProvenance` serialize and round-trip populated Phase 13 records; manifest references point to source artifact records it depends on; populated records reject non-plain data and unknown malformed fields according to local conventions; inspection stage records remain stable/additive and artifact-safe; contract tests explicitly assert no raw source bytes, resolved resolver outputs, constructed runtime objects, or fully resolved config payloads appear in default artifact records.

### Integration Suite

- Status: required.
- Expected paths: `tests/integration/config/test_compose_config.py`, `tests/integration/config/test_compose_includes.py`, `tests/integration/config/test_compose_overrides.py`, `tests/integration/config/test_compose_recipes.py`, `tests/integration/config/test_compose_resolvers.py`, and possibly `tests/integration/config/test_compose_provenance.py` if a new focused file improves reviewability.
- Required assertions or deferral reason: public `compose_config(...)` with base, overlays, recursive includes, user include replacement, recipes, ordinary overrides, and resolver expressions returns populated manifest/provenance/source records; source artifact references cover base/overlay/include roles and recipe references where available; redacted/unresolved artifacts contain no resolved `oc.env` values; resolver expressions are preserved in artifact-safe records; plaintext secret override docs/warnings are covered; inspection and composed config agree on artifact records.

### E2E Suite

- Status: required but limited.
- Expected paths: existing or new domain-neutral public composition coverage under `tests/e2e/` only if it can avoid CLI, runner, store, and pipeline execution behavior.
- Required assertions or deferral reason: use public `compose_config(...)` on a small synthetic config tree and assert populated artifact records are present, composition markers are absent from unresolved/redacted output, and default artifacts contain no resolved runtime values. If the existing e2e harness is runner/pipeline-specific, record broader e2e artifact persistence as deferred to Phase 16 and cover Phase 13 through integration tests.
...
- Safe implementation slices: start with contract/unit tests for populated artifact shapes; add source artifact population; add manifest population; switch redacted artifact population to unresolved/redacted-safe data; enrich provenance/inspection payloads; add integration coverage and the secret override docs snippet.
...
- Decisions the executor must not revisit: `loom.config` remains persistence-free; `loom.pipeline` must not depend on config or manifests; `_copy_` stays unsupported; default artifacts do not contain raw source bytes or resolved runtime values; resolver outputs are not persisted by default; v1 has no CLI; no plugin/remote/global include resolvers; no final fingerprint comparison or resume algorithm; no raw source snapshot opt-in or dedupe; no resolved config persistence.
- Conditions that require stopping for the manager: satisfying the phase appears to require persistence, raw snapshots, final fingerprint comparison, CLI/store behavior, pipeline ownership changes, resolved runtime value artifacts, raw source bytes in default records, public resolver/include policy changes, or reopening the already-used plan quality gate.
- Expanded-path refinement notes: draft pass only. The refine pass must confirm the artifact reference shape, redaction serialization boundary, recipe-source fallback, and pre-submit blocker gate expectations before implementation begins.
