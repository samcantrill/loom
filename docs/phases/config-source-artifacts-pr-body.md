...
- Propagated nested include records and raw source capture from user include replacement targets so replacement files that include local/file leaves are reconstructable from returned snapshots.
...
- Integration tests cover default metadata-only behavior, opt-in reconstructability for local/file sources, nested user replacement include snapshots, recipe metadata-only behavior, and default fingerprint exclusion of raw payloads.

## Tests And Validation

| Check | Result | Evidence |
| --- | --- | --- |
| Focused source snapshot regression | Passed | 5 tests passed in `tests/integration/config/test_compose_source_snapshots.py`. |
...
| e2e | passed | 5 | 0 | 0 |
| config-extra | passed | 301 | 0 | 434 |
| Overall | passed | 735 | 9 | 434 |
