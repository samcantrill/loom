...
- Refine pass: required by expanded path because structured error context is a public-ish contract for later composition phases and tooling; pending and not consumed by this planning-only draft pass.
...
- Keep public exceptions as `ConfigError` subclasses, including `ConfigLoadError`; add narrow subclasses for unsupported directives only if that improves contract clarity without broad API churn.
...
- Include source path, config source kind, source order, config path where known, expected/actual shape where useful, and remediation guidance in structured error context.

## Out-of-Scope Work

- `_include_` implementation, include resolution, include stacks, include provenance records, include cycle detection, and include-related error subclasses beyond reserving compatible context fields.
- `_replace_` handling beyond not confusing it with `_copy_`; strict `_replace_` behavior belongs to Phase 3 and include phases.
- Overlay merge behavior beyond the existing `load_config(..., kind, order)` source context.
...
- Structured error context is the public-ish boundary for this phase. It must be inspectable without parsing strings and serializable as plain data, with stable additive field names for source path/kind/order, config path, directive, expected shape, actual shape, reason/code, and remediation where available.
...
| Existing v0 compose tests may still assert pre-v1 reserved-key behavior until orchestration phases change them | Phase 2 should not rewire public `compose_config` order beyond loader behavior. | Revisit in Phase 12 when public orchestration owns complete v1 behavior. |
...
- Scope-control checks: no include modules or resolution behavior; no override or merge primitive behavior; no schema validation; no resolver execution; no run-store writes; no CLI commands; no root exports; no changes that make pipeline import config.

## Implementation Steps

1. Define the structured config-error context shape and serialization helpers in `src/loom/config/errors.py`, preserving existing exception subclass compatibility.
...
4. Extend unit tests for loader success/failure cases and structured error subclass/context behavior.
5. Add contract coverage for structured error context serialization and redaction-safe field expectations.
6. Run targeted loader/error/contract/package checks before handing to PR preparation.
...
- Rejecting `_copy_` during loading may affect existing v0 tests that previously treated `_copy_` as ordinary data; update only tests whose behavior is intentionally superseded by the accepted v1 decision.
