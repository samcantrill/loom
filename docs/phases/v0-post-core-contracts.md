...
The phase fixes findings 5, 6, and 10 from the source plan: shallow immutability, persisted schema boilerplate, and hard config dependencies. It must not implement store capability rewrites, `ArtifactAddress`, run-scoped artifact store changes, stage factory blocks, semantic fingerprint changes, runtime events, lock behavior, planner explanations, recipe catalog policy, or runner lifecycle decomposition.
...
- Migrate a focused set of existing persisted readers to the shared schema helpers so the new API is proven by real call sites and tests. Candidate readers are manifests, status records, provenance documents, planning models, and local store wrappers; the refine pass should choose a reviewable subset and identify any intentional deferrals.
...
- No store/context capability rewrite, run-scoped artifact-store contract change, `ArtifactAddress`, run-document/user-metadata API rename, or local path contract break.
...
- The implementation may add a new config-extra suite target rather than forcing all config tests into the default no-extra suite.
...
This draft records closed source-plan decisions but leaves lower-level function names and migration targets for the refine pass.
...
- `to_dict()` and serialization helper output must remain mutable plain data.
...
- Known older document versions migrate through explicit document-owned migration tables or dispatch functions.
- There is no global process-wide migration registry.
- Config dependencies are optional under a `config` extra, not hard runtime dependencies.
- Default validation must prove no-extra behavior, and config-extra validation must run config behavior with the optional dependencies installed.
...
| Config package lazy-import behavior may be transitional. | The phase should avoid leaking optional dependencies without redesigning future stage factory/target resolution. | Revisit in Phase 3 when stage factory and import-safe construction are implemented. |
...
- Files and areas to inspect:
  ...
  - focused persisted-reader modules selected during refinement
  - `src/loom/config/`
...
1. Add or refine tests that demonstrate the desired immutable internals and mutable `to_dict()` output for refs, artifacts, records/manifests/views, output specs, stage specs, and pipeline specs.
2. Add serialization freeze/thaw helpers for plain structured data and expose only the helpers needed by core modules.
3. Apply recursive immutable normalization across the in-scope frozen value objects and update `to_dict()` methods to thaw recursively.
4. Expand `serialization/schema.py` with strict persisted-document helpers and migration dispatch primitives.
5. Migrate the refine-selected persisted-reader call sites to the shared schema helpers and add compatibility tests for unknown fields, unsupported versions, and explicit older-version migration dispatch.
6. Move OmegaConf, Pydantic, and PyYAML into the `config` optional extra and adjust config package imports so no-extra core imports stay clean and config behavior fails clearly without extras.
7. Update package/import tests, optional-dependency markers, Makefile targets, and test harness summary output for separate no-extra and config-extra evidence.
8. Update structure, serialization, and config docs for the changed ownership and dependency policy.
9. Run targeted package/unit/config-extra checks during implementation, then leave final `make validate-pr` and `make test-summary` evidence for PR preparation.
...
- Expected paths:
  ...
  - config API package tests split or marked so no-extra and config-extra expectations are explicit.
- Required assertions:
  ...
  - Config public API behavior is covered in the config-extra path, or no-extra config imports fail with a clear config-owned missing-extra error.
...
- Expected paths:
  ...
  - selected persisted-reader unit tests for status, planning, provenance, manifests, or local store documents.
- Required assertions:
  ...
  - Known older-version examples dispatch through explicit document-owned migrations.
...
- Required assertions or deferral reason:
  - Phase 1 adds shared helper behavior rather than a new extension protocol. Add contract tests only if the refine pass chooses to expose a reusable persisted-document protocol; otherwise record existing contract suite evidence through `make test-summary`.
...
- Required assertions:
  - Add a config-extra target or equivalent opt-in harness path that runs config-marked package/unit/integration/docs tests with `uv run --extra config` or another isolated config-extra environment.
  - Add a no-extra target or equivalent harness path that proves default imports without the config extra.
...
- Over-migrating persisted readers to the new schema API could turn Phase 1 into a broad refactor. The refine pass should choose a small migration set and record deferrals.
...
The refine pass must confirm or adjust the exact target names after the harness design is finalized. The implementation must still provide explicit default no-extra evidence and config-extra evidence.
...
PR evidence must also include the explicit no-extra and config-extra validation outputs if they are not both covered directly by those final commands.

## Handoff Notes For `loom_phase_executor`

- Safe implementation slices:
  - Tests and serialization freeze/thaw helpers.
  - Core value-object immutability updates.
  - Shared schema helper API and focused persisted-reader migration.
  - Optional config extra packaging and import-boundary tests.
  - Harness/Makefile summary evidence.
  - Documentation updates.
- Tests to run with each slice:
  - Core value-object unit tests after immutability changes.
  - Serialization schema/plain unit tests after helper changes.
  - Package import-boundary tests after packaging or import changes.
  - Config-extra unit/integration tests after moving dependencies.
...
- Decide the exact immutable helper names and public/private export policy.
- Decide which persisted-reader modules are migrated in Phase 1 and which are intentionally deferred.
- Make the no-extra/config-extra harness design command-complete, including whether isolated uv environments are needed to prevent optional dependency contamination.
- Confirm whether config package import without extras should succeed with lazy missing-extra errors or fail immediately with a clear optional-dependency message.
- Convert the implementation steps into decision-complete file-level instructions and update validation commands to the exact final Make targets.
