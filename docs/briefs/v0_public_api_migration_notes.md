...
| Direct stage target values in local run state documents, including `StageSpec.target_path` | `StageSpec.factory.target_path` and `StageFactorySpec` as the explicit parsing/validation contract |
...
| Artifact-store operations that accepted a `run_id` parameter | Run-scoped `ArtifactStore` instances. Use `save`, `register`, `load`, `exists`, `verify_checksum`, and `validate` against `ArtifactRef` values for the bound run |
| Ambiguous `read_run_metadata()` style API for the full `run.json` document | `read_run_document(run_id)` for the persisted run wrapper and `read_run_user_metadata(run_id)` / `write_run_user_metadata(run_id, metadata)` for user-authored metadata |
