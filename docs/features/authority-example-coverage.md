...
| --- | --- | --- |
| Authority lifecycle | `loom authority start`, `status`, `doctor`, `restart`, and two completed `stop` calls use an explicit state directory and workspace registry. | `operations.authority-lifecycle` |
