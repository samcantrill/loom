...
| coordinator client | The native Python/CLI integration surface for direct coordinator control through a Unix socket or authenticated HTTPS, using existing admission, operation and inspection models. | The connection selects transport; it does not register a worker, start a service or own submitted work. See [the client guide](features/coordinator-client.md). |
| expected coordinator ID | The optional stable coordinator identity carried on a control request and checked by the receiving coordinator before lookup or mutation. | Use it with saved operation/admission/queue IDs after reconnect. A prior handshake is not a substitute for this request guard. |
| preparation operation | Coordinator-owned accepted intent linking a captured input, one managed preparation child and canonical target publication. | Acceptance or child success does not establish publication; only `applied` provides a prepared receipt. Submission is separate. |
| preparation profile | A protected alias selecting one qualified existing resident software profile, allowed source roots/modes and native child runtime options. | Not an environment installer or a reservation of target execution capacity. |
| shared preparation input | A finite immutable snapshot of explicitly selected regular files, addressed through a protected root alias and verified manifest. | Not a live checkout, general resource URI or automatic delivery of target runtime inputs. |
| mutation outcome | Structured client evidence distinguishing an authoritatively applied or unapplied mutation from one whose commitment is unknown; null for a read. | A lost reply can be unknown. Reconcile with the original IDs and exact intent; observation timeout does not authorize cancellation or a fresh submission. |
