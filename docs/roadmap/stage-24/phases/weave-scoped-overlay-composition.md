...
- Refine pass: pending for expanded path before implementation
...
- Blockers: no blocker to drafting; implementation must wait for the pending expanded-path refine pass to finalize this artifact.

## Objective

Integrate Phase 1 scoped overlay records into internal config composition so argv-scoped overlays load, merge, and audit like authored overlay-family sources at the confirmed insertion point, while preserving source/provenance records, manifests, raw snapshot references where applicable, artifact-safe fingerprints, and existing non-argv composition behavior.
...
This phase must keep direct `compose_config(...)` and `inspect_config_composition(...)` callers behavior-compatible. It may add internal helpers or private parameters used by later public wrappers, but it must not expose a public argv API.
...
- Required checkpoints: direct non-argv composition contracts unchanged; scoped overlay values have correct final authorship; manifest, provenance metadata, source artifacts, raw snapshot references, and artifact-safe fingerprint metadata include scoped overlay facts.
...
- Existing files or modules that constrain this phase:
  ...
  - `packages/weave/src/weave/fingerprints.py` builds artifact-safe fingerprint facts from source artifacts while avoiding raw source bytes and runtime resolver outputs.
...
- Add private composition plumbing that can accept Phase 1 `ArgvScopedOverlay` records without changing public `compose_config(...)` or `inspect_config_composition(...)` signatures.
...
- Add internal inspection plumbing for an argv-only stage named `argv_scoped_overlays` immediately after `file_include_expansion`.
...
- New persisted `SourceArtifactRecord.kind`; if a new kind appears necessary, stop for manager review and plan refinement before implementation continues.
...
- Overlay-family source artifacts with explicit metadata are sufficient for auditability and fingerprinting in this phase.
- The executor may choose the smallest private helper shape that keeps direct public composition APIs unchanged and lets Phase 3 wrap it.

## Scope Contract

No public API changes are in scope. The internal contract is: a sequence of already-resolved `ArgvScopedOverlay` records can be provided to private composition plumbing; each record has a non-empty scope path, operation `update` or `add`, and a resolved file path. The composition must treat those records as trusted authored config input, load their YAML as mappings, and apply them only at the requested scope.
...
The public non-argv inspection contract must not change. The argv-only inspection path may include `argv_scoped_overlays`, but public exposure waits for Phase 3.
...
- Future Phase 3 helpers should be able to call the internal composition path without reimplementing merge/provenance/fingerprint logic.
- Existing artifact-safe fingerprint policy must remain path-portable and must not include raw source bytes or resolved runtime values.
...
- Any need for a new source artifact kind, manifest schema version, or changed non-argv inspection stage tuple is a stop condition.
...
| Change `compose_config(...)` or `inspect_config_composition(...)` public signatures | Direct callers must see no behavior change; Phase 2 is internal composition plumbing. |
...
- Expected PR size and shape: medium `packages/weave` PR with one private composition path, scoped merge/source-map support, artifact/fingerprint metadata additions, and focused tests.
...
- Scope-control checks: no public argv helper/export, no docs/features edits, no Loom CLI imports, no new artifact kind, no changed non-argv inspection stage tuple, no broad rewrite of recipe/include/override behavior.

## Implementation Steps

1. Add private composition input/plumbing for scoped overlay records and thread it through the existing composition orchestration without changing public compose or inspect signatures.
...
5. Add internal `argv_scoped_overlays` inspection stage support for the private argv path while keeping public non-argv inspection stages unchanged.
...
- Status: required
- Expected paths: new `packages/weave/tests/integration/config/test_compose_argv_scoped_overlays.py`, plus `packages/weave/tests/integration/config/test_compose_recipes.py`, `packages/weave/tests/integration/config/test_compose_overrides.py`, and `packages/weave/tests/integration/config/test_compose_source_snapshots.py`.
- Required assertions or deferral reason: `data/=data_A`, `model/=model_B`, `model/pipeline/=pipeline_A`, `+runtime/=local`, and `_replace_: true` merge correctly; scoped overlays apply after file includes and before recipe expansion; value overrides still win after recipe expansion; update/add target errors are structured; raw snapshot opt-in records scoped overlay files where applicable.
...
- Required assertions or deferral reason: contract tests run under existing optional dependency setup; raw snapshot opt-in proves scoped overlay source references can become available payloads while default artifacts stay metadata-only.
...
- Stop if a new source kind, manifest schema change, public API change, or non-argv inspection contract change appears necessary.
...
uv run pytest packages/weave/tests/integration/config/test_compose_recipes.py packages/weave/tests/integration/config/test_compose_overrides.py packages/weave/tests/integration/config/test_compose_source_snapshots.py
...
- Decisions the executor must not revisit: no public argv helper/export, no first-party CLI integration, no warning UX, no new source artifact kind without stopping, no changes to direct non-argv inspection stage tuple.
- Conditions that require stopping for the manager: schema-versioned manifest/source kind seems necessary; `compose_config(...)` signature would need to change; scoped overlay facts cannot be artifact-safe; public `inspect_config_from_argv(...)` exposure is needed to test the stage; validation shows broad unrelated regressions.
- Expanded-path refinement notes: the refine pass should tighten any helper naming or contract details after this draft, but should not expand into Phase 3 public API work.
...
- Remaining blockers: implementation must wait for the pending refine pass.
