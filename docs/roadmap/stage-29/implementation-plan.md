...
Status: production correction active; Phase 10 merged, Phase 11 active, and
Phase 12 pending;
...
Current phase: Phase 11 `pr_open`
...
| 11 | `resident-agent-correctness-security` | pr_open | `docs/roadmap/stage-29/phases/resident-agent-correctness-security.md` | `agent/stage-29-p11-resident-agent-correctness-security` | [#253](https://github.com/samcantrill/loom/pull/253) open | Mandatory managed execution requirement; one candidate per agent resident profile; exact pinned profile target; allowlisted worker environment; explicit agent provider composition; planner/provider contract startup validation; public provider conformance check | Fresh full validation, durable test summary, and manager-local review pass; target, title, scope, and mergeability are verified. |
