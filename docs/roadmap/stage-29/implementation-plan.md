...
Current phase: Phase 9D2 `in_progress`
...
| 9D2 | `embedded-release-replay-closure` | in_progress | `docs/roadmap/stage-29/phases/embedded-release-replay-closure.md` | `agent/stage-29-p9d2-embedded-release-replay-closure` | pending | Selective Phase 9D reuse; saved availability-revision replay after fresh observation; final event acknowledgement before coordinator release; identical definitive-decline ordering; causal crash-cut proof | Merge the complete embedded/local consumer only after final release remains discoverable and exact across every supported restart boundary. |
...
| 9D2 | Dedicated branch/worktree active from `00ce27f`; PR pending | Manager-local lean plan passed; maintainer approved exact saved-revision/event-order closure | Must preserve all validated hard-cut Phase 9D behavior while changing only release replay and proving both terminal/decline crash cuts | Implementation pending one executor; correction budget 0/3 |
