...
| 15 | `management-cli-examples` | in_progress | `docs/roadmap/stage-29/phases/management-cli-examples.md` | `agent/stage-29-p15-management-cli-examples` | pending | Implementation `6907c14`: per-admission semantic revisions; bounded concurrent long polls; bounded admission/agent/operation reads and CLI; portable local-owner operator policy; accepted-time/health/replay corrections; three managed-operation journeys | Fresh local gates passed: `make validate-pr`; `build/test-summary.md` reports 2,929 passed, 3 skipped, 0 failures/errors. Required independent review, PR, merge, and cleanup remain manager-owned. |
