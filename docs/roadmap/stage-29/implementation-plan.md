...
| 11 | `resident-agent-correctness-security` | in_progress | `docs/roadmap/stage-29/phases/resident-agent-correctness-security.md` | `agent/stage-29-p11-resident-agent-correctness-security` | not opened | Mandatory managed execution requirement; one candidate per agent resident profile; exact pinned profile target; allowlisted worker environment; explicit agent provider composition; planner/provider contract startup validation; public provider conformance check | Implementation and targeted gates pass at `77510e7`; full validation and PR preparation are pending. |
