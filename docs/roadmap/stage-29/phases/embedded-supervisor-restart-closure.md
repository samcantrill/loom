...
- Worktree path: create after Phase 9C is remotely merged
- Base revision: current `origin/develop` after the Phase 9C merge
- PR target: `develop`
- PR title: `feat(scheduling): close embedded managed restart`
- Dependency: Phase 9C remotely merged. Blocked Phases 9–9B remain read-only
  evidence; Phase 9C's private supervisor is the only process owner.
...
Move the embedded/local production managed path onto Phase 9C's separate
...
- Fresh local initialization creates a one-member Phase 9C supervisor profile
...
- Route embedded launch/query/stop/result through the Phase 9C client and finish
...
- Remote supervisor behavior already merged in Phase 9C except shared private
...
resident-assignment bundle decision` and Phase 9C's current remote schema. It
...
parent reads prepared StageWorkerRequest and source ArtifactRefs
  ...
  -> Phase 9C supervisor launches fixed resident worker
...
```text
matching Phase 9C supervisor/profile verified
...
reuse. `CONTAINED` follows only Phase 9C's positive group proof.
...
| One launch survives local agent restart | Phase 9C supervisor | Duplicate stage effects | Running-worker restart sentinel |
...
1. Complete explicit local library/CLI profile hard cut and one-member supervisor
   initialization/open composition using the merged Phase 9C service.
...
- Integration: real Phase 9C service plus fresh local agent applications; crash
...
- Stop for a genuinely new public/durable choice or if embedded input/output
  cannot use the fixed resident bundle. Do not reopen Phase 9C or implement 9E/F.
- Independent review must verify owner removal, exact projection, hard cut, and
  fresh-process local restart.

## Executor Handoff

- Start only after Phase 9C remotely merges and base current `origin/develop`.
- Read this plan, Phase 9C completion, and blocked Phase 9A's exact shared-bundle
...
- Manager preparation: pending Phase 9C merge
