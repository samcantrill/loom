...
- Workflow path: expanded because this phase establishes subsystem-public
  extension contracts and one new durable coordinator projection
...
- Vertical outcome: Loom has one authoritative answer to “which exact stage
  ...
  resource snapshot without reserving capacity, binding an attempt, or launching
  a process.
...
- Later work explicitly out of scope: Phase 2 owns every side effect from
  logical reservation through local process release. Phase 3 owns daemon/public
  facade migration. Phases 4–8 own remote trust, transport, GPU placement,
  controls, and recovery.

This is the one deliberate foundation phase in the eight-phase plan. Its
boundary is safe to merge independently because scheduling is pure and stage
work is a rebuildable projection. Existing execution remains callable, but all
current and future managed paths must consume the extracted readiness predicate
rather than retain a second DAG interpretation.
...
- `loom.scheduling` must remain import-light. It may depend on core typed values
  and standard-library protocols, but not queue repositories, authority clients,
  SQLite, artifacts, routes, processes, executors, project code, vendor SDKs, or
  CLI modules.
...
- Add one `ResolvedStagePlacement` construction path that combines:
  ...
  - run and pool defaults, including `max_parallel_stages`;
...
- Keep authored/runtime resource validation owned by the existing
  `ResourceValidator` contract. A `ResourcePlanner` receives canonical validated
  entries and owns scheduling-time merge, feasibility proposals, exact claims,
  and safe failure explanations. Durable resolved resources retain validator
  and planner descriptors separately.
...
- Provide deterministic built-in CPU and memory planners, built-in target/
  attribute hard rules, neutral/default preference behavior, and default
  FIFO-with-safe-bypass scheduling policy. CPU accepts positive integers only;
  memory normalizes to integer bytes.
...
- Add a durable `RunOrchestrator` reconciliation step that:
  - resolves REUSE, SKIP, and blocked actions without agent capacity;
  - prepares only ready `PlanAction.RUN` attempts idempotently;
...
if ready.action is PlanAction.RUN:
    coordinator.reconcile_stage_work(ready, resolved_placement)
```
...
soon as any input version changes.
...
outcomes, identity separation, readiness ownership, or the no-mutation boundary.
...
- Existing seams reused: `ResourceRequest`, validators, execution plans,
  ...
  reports.
- Material additions: one pure scheduling package and one rebuildable durable
  projection, each required by the accepted per-stage/global scheduler.
...
3. Extract the shared readiness predicate and route existing runner readiness
   through it; add controller-action reconciliation and the durable
   `StageWorkRecord`/store operations.
...
| --- | --- | --- | --- |
| Package | Required | Cheap intentional exports and dependency direction | Import `loom.scheduling` and `loom.testing` without runtime/network/database imports; no root re-export |
...
| Integration | Required | Authority readiness and durable projection | Train/evaluate, diamond, reuse/skip/failure/retry, per-run parallel limit, restart/rebuild |
...
- Review focus: import direction, immutable closed values, explicit registry
  composition, kernel result validation, readiness extraction, and absence of
  side effects.
- Stop if: canonical runtime requests cannot be composed without weakening;
  ...
  need to own lifecycle truth; a protocol requires a live store/launcher; or a
  required quantity cannot be represented exactly.
...
- Decisions not to revisit: fixed kernel, narrow protocols, explicit frozen
  ...
  readiness predicate, rebuildable stage work, and no side effects.
