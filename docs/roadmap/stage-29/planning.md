...
Status: implementation blocked; Phases 1-3D, 4A, 5A, and 6 merged; Phases
...
The hard cut-over remains approved, but a fresh Phase 7 recovery cannot be
shaped until its external bootstrap-capability delivery boundary is selected.
Current gate: Phase 7 is blocked read-only evidence after PR #242 passed local
and CI gates but failed required review. Phases 8 and 9 cannot start.
Blockers: Loom has no repository-native channel that releases one opaque
assignment capability only to the allocated SLURM job outside scripts,
arguments, scheduler metadata, and environment. The maintainer must select the
supported site-owned delivery mechanism before recovery planning resumes.
...
| Phase 3 execution evidence | Phase 3A never reached a production worker. Phase 3B reached the real Phase 1/2 path but failed five contracts. Phase 3C closed those five but review found two residual failures. Fresh Phase 3D closed complete owner evidence, missing/substituted-store handling, and partial-start cleanup, then passed 2,538 categorized tests, `make validate-pr`, independent review after correction 2/3, and CI. | None. PR #237 squash-merged as `6a8cf9f`; Phase 3A-3C remain read-only blocked evidence. | Preserve Phase 3D in the merged lineage and retain Phase 3A-3C as blocked evidence. |
| Phase 4 execution evidence | Candidate `c373d04` establishes the complete no-launch mTLS/session/offer/poll path and passed 2,554 categorized tests plus CI. Review proved that any credential holder could invent the syntactically valid agent-side retirement digest, and that two principals choosing the same poll ID collide in SQLite. | Correction 3/3 is exhausted; PR #238 is closed and the branch is read-only evidence. | Preserve Phase 4 as blocked evidence; use only merged Phase 4A behavior downstream. |
| Phase 4A execution evidence | The fresh hard-cutover recovery journals one secret per session, retains only its verifier coordinator-side, verifies retirement before mutation, scopes poll identity by principal, and rejects the unmerged candidate schema. Source/test revision `41a6045` passed 2,557 categorized tests and the full gate; independent review and CI passed. | None. PR #239 squash-merged as `2d273b8`. | Preserve the merged session and poll-identity trust boundary downstream. |
| Phase 5 execution evidence | Candidate `d536a1e` completed the remote CPU/memory assignment, relay, execution, result, and release path and passed 2,576 categorized tests plus the full gate. Review found that both transfer owners could publish final bytes before the matching SQLite commit, so a crash could strand exact replay. | Correction 3/3 was exhausted; the branch remains blocked read-only evidence. | Preserve Phase 5 as evidence; use only merged Phase 5A behavior downstream. |
...
| Phase 7 recovery decision | Submit isolation, scheduling continuation, restart coverage, and assignment-bound verifier/consumption have existing owners. Secret delivery does not: profile mTLS is shared transport identity and Phase 5 relay begins only after authorization. | Loom needs a supported site-owned mechanism that makes one opaque assignment capability available only to the allocated job without putting secret bytes in scripts, arguments, scheduler metadata, or environment. No repository-native channel satisfies that contract. | Select the concrete site delivery boundary before shaping a fresh Phase 7 recovery. Phases 8 and 9 remain blocked. |
| Approval | The maintainer approved the Stage 29 hard cut-over and explicitly rejected candidate-schema compatibility or migration. Phase 7 recovery must likewise start fresh and reject the unmerged candidate. | The site-owned allocated-job capability-delivery mechanism is not selected; that is a material deployment/trust decision rather than repository mechanics. | Obtain one maintainer decision, then shape the smallest fresh Phase 7 recovery around it. |
...
| Blocked Phase 4 candidate and independent review | Validated source/test revision `c373d04` adds the real outbound-agent journal, mTLS application boundary, durable session replay, exact offers, held polls, additive Phase 3 root upgrade, and causal no-launch gates. Review showed that copied session fields plus an arbitrary 64-character reference digest do not prove possession of the original agent journal, and that `poll_id TEXT PRIMARY KEY` conflicts across authorized principals even though lookups are principal-scoped. | Reuse the validated production path without its workflow metadata. Add one fresh secret/verifier commitment per registration and verify the revealed secret before retirement fencing or mutation; key every coordinator poll operation by `(principal_id, poll_id)`. Reject the unmerged candidate schema rather than translating it. | FR-11, FR-16, FR-17, FR-20, FR-21, FR-25; DQ-22 |
...
| Blocked Phase 7 candidate and independent review | Candidate `3515400` supplies the distinct ready-stage profile/dispatcher/bootstrap path and passed local/CI gates. Review showed that the production submit subprocess inherits `SBATCH_*` overrides and secret-bearing daemon variables; profile mTLS plus caller-selected operation/digest/handle can claim another same-profile assignment; the first unavailable route returns before independent work is considered; and restart tests do not reconstruct fresh coordinator processes across retained submission states. The repository contains no job-private capability-delivery channel, and Phase 5 relay cannot bootstrap one because it is authorized only after registration. | Reuse the candidate selectively only after a site channel is selected. Isolate ready-stage submission with a bounded protected allowlist plus `--export=NIL`; bind a provider-delivered one-use capability and retained bootstrap session to assignment/operation/request/issuer/policy/handle/incarnation; continue after route-local waits; and prove fresh-process no-resubmit. Keep historical whole-run SLURM unchanged and reject every unmerged candidate schema. | FR-4, FR-18, FR-20, FR-25, FR-27–FR-30 |
...
| FR-21 | Agent drain/reload withdraws availability before changing agent-owned pools, providers, or resident capabilities. Live claims keep their original agent configuration/inventory/provider identity until release. Coordinator scheduling-policy/component reload is a separate owner-local transaction retaining descriptors referenced by pending work and assignments; there is no distributed configuration swap, and temporary contract skew makes candidates ineligible. Session replacement requires graceful retirement with original-journal possession proven by its fresh per-session secret, or complete positive-containment evidence. | Reconfiguration cannot mutate resources under live work or let an agent reload reinterpret coordinator-owned planners/rules/policy. A credential, copied request body, or replacement empty root alone cannot retire an old session. | Agent/coordinator reload-skew, missing/wrong-secret no-mutation, and session tests. | locked |
...
registration operation ID/digest and one fresh random retirement secret before
send. The request carries only the secret's SHA-256 verifier, and the
coordinator stores only that verifier with the session. A crash or lost response
replays the same request, verifier, and coordinator-issued ID; the agent
persists the returned session and keeps the secret in its protected journal
before offering capacity. A reconnect
...
transfer/result/output/sequenced-event/outbox set is empty. Retirement reveals
the one-session secret over mTLS with the complete bound evidence; the
coordinator constant-time checks its SHA-256 value against the stored verifier
before withdrawing an offer, fencing a poll, or changing session state. The
...
fact, only Phase 9's positive-containment replacement may create a new session.
...
Certificate issuance, organization-wide identity federation, general
application-layer message signing, at-rest encryption, and hostile-workload
sandboxing remain explicit deployment or future concerns. The one-time
retirement preimage is narrower: it proves possession of the original protected
session journal and cannot authorize any other operation or later session.
...
Phase 7 implementation evidence proved that profile-wide mTLS is transport
identity, not assignment authority. The assignment bootstrap additionally needs
one opaque one-use capability bound to assignment, submission operation,
request digest, issuer epoch, profile/policy revision, scheduler handle, and
bootstrap incarnation. Its verifier/consumption and exact replay belong to the
coordinator's SLURM assignment owner. Secret delivery belongs to a protected
site mechanism that releases the capability only to the allocated job; the
secret may not appear in the batch script, arguments, scheduler metadata, or
environment. Loom currently has no such native channel, and the artifact relay
cannot provide it because relay authorization follows bootstrap registration.
Until a supported scheduler/site-managed job-private file, credential broker,
or equivalent channel is selected, the ready-stage route remains blocked.
...
| DQ-22 | FR-10, FR-11, FR-16, FR-20, FR-21, FR-25 | The agent atomically persists a registration operation and fresh per-session retirement secret before send; registration carries only its SHA-256 verifier. The coordinator creates the session durably/idempotently and stores only the verifier, while the agent persists the returned session before offer. Cooperative retirement must reveal the secret with complete bound empty-reference evidence; the coordinator checks the verifier in constant time before any fencing or mutation. Otherwise require Phase 9 positive-containment replacement and retain an old-session tombstone. | Connection loss, credential rotation, a crash/lost registration response, a credential holder without the original journal, or a fresh local database cannot prove that old work stopped or justify a caller-selected replacement identity. A new session gets a new secret, so revealing the old one cannot weaken later sessions. | The agent journal retains one protected secret until retirement outcome is durably acknowledged; lost-state agents remain blocked until privileged recovery. | locked |
...
| A credential holder could forge the agent half of clean retirement | FR-11, FR-16, FR-20, FR-21, FR-25; DQ-22 | Phase 4 candidate `c373d04` checks copied session fields and accepts any syntactically valid reference digest, so direct HTTP can retire without possessing the original protected journal. False retirement would let a later session replace unresolved work. | In fresh Phase 4A, journal one random secret per registration, send/store only its verifier, reveal and verify it before any retirement mutation, keep raw secret out of coordinator persistence/status/errors/audit, and rotate it for every new session. | maintainer-approved; removal-first safety review passed |
| Poll identity was scoped in reads but global in SQLite | FR-17, FR-25 | Phase 4 candidate `agent_polls.poll_id` is the primary key even though the protocol permits independently authorized principals to choose operation IDs. A shared ID causes a permanent insert collision and indeterminate 5xx. | Make `(principal_id, poll_id)` the primary key and include the principal in every single-poll read, completion, failure, and cleanup; session-wide fencing remains session-scoped. | maintainer-approved; removal-first safety review passed |
...
| Clean or guarded session replacement | Before registration send, the original journal durably owns a fresh one-session secret while the coordinator receives/stores only its verifier and idempotently allocates the session. Cooperative complete-empty retirement reveals that secret and is rejected before mutation when it is missing/wrong; unresolved or lost old state cannot roll over without complete positive containment. | Agent registration journal + coordinator session/retirement owner. | Lost-registration-response verifier replay, different secret per new session, direct HTTP wrong/missing-secret no-mutation, original-root success, credential rotation/restart, empty/unresolved/old-journal refusal, complete request/claim/control/transfer/result/event/outbox set, secret-redaction, exact retirement replay, and late-message tests. | approved for Phase 4A |
...
| 3D — local-daemon status and restart closure | The validated persistent daemon path reports complete owner evidence and never resumes retained work from missing or substituted owner state. | Selectively reuse Phase 3C source/tests; add aggregate state plus owner-derived revision/receipt and freshness to every healthy scheduling/assignment/local-agent axis; initialize and bind current owner stores to stable roots; require expected stores before retained scheduling and degrade/fail closed if they disappear or are substituted; clean up locks/workers when execution construction fails. No compatibility, migration, remote recovery, process adoption, or new public extension surface. | Phase 2 plus read-only Phase 3A-3C evidence. | 2,538 categorized passes, `make validate-pr`, independent review after correction 2/3, and CI; PR #237 squash-merged as `6a8cf9f`. | merged |
| 4 — blocked authenticated-agent candidate | Candidate `c373d04` proves most of the outbound no-launch session path but cannot merge. | Evidence only. Its clean-retirement digest does not prove possession of the original journal, and its globally keyed poll table conflicts across principals. | Phase 3D. | 2,554 categorized passes, `make validate-pr`, CI, and required independent review; PR #238 closed after correction 3/3 was exhausted. | blocked |
| 4A — authenticated-agent trust closure | Outbound agents on `machine-A` and `machine-B` authenticate, register/reconcile, publish bounded offers, and request work through a no-launch transport gate, with clean retirement provably tied to the original protected session journal. | Selectively reuse the blocked candidate; add one fresh per-session secret/verifier commitment, verify it before retirement mutation, keep it out of coordinator persistence/observability, and make poll identity `(principal_id, poll_id)`. Retain mTLS/current-policy scopes, exact replay, current-epoch offers, additive Phase 3 root upgrade, hard rejection of unmerged candidate schemas, and causal no-launch gates. No assignment delivery, artifact bytes, remote process launch, general message signing, migration, or compatibility adapter. | Phase 3D plus read-only Phase 4 evidence. | Source/test revision `41a6045` passed the full gate and 2,557 categorized tests; independent review and CI passed; PR #239 squash-merged as `2d273b8`. | merged |
| 5 — blocked remote CPU/memory candidate | Candidate `d536a1e` completes authenticated remote assignment, bounded regular-file relay, execution, result replay, and ordered release but cannot merge. | Evidence only. Input/output publication can survive while the matching SQLite finalization rolls back, leaving exact replay without its staging file. | Phase 4A. | 2,576 categorized passes and the full local gate succeeded; required review found the publication/finalization crash window after correction 3/3. | blocked |
...
Nine numbered phases plus Phase 3B, Phase 3C, Phase 3D, Phase 4A, and Phase 5A
...
in one independently reviewed PR. Phase 4 then exhausted correction 3/3 after
its required review found two narrow trust/identity failures. Phase 4A likewise
starts from current `develop`, treats Phase 4 as read-only evidence, and owns
only the per-session retirement possession proof, principal-scoped poll key,
...
gap. A Phase 7 recovery subphase is deliberately not shaped yet: unlike the
repository-owned corrections, secure capability delivery requires a concrete
site-owned job-private channel and planning cannot invent that deployment
contract. Phases 8 and 9 remain pending behind the unresolved choice.
...
| Phase 3 recovery evidence | Phase 3A lacked a production trace. Phase 3B supplied it but failed five contracts. Phase 3C closed those five and passed local/CI gates; review then identified two residual failures. Phase 3D closed complete healthy-axis evidence, missing/substituted-store handling, and localized startup cleanup; full local gates, independent review, and CI passed before PR #237 merged. | pass |
| Phase 4 recovery evidence | Candidate `c373d04` passed the full local gate and PR #238 passed CI. Review demonstrated a forgeable retirement proof and global poll key. Fresh Phase 4A gave journal possession and poll isolation one authoritative owner each without general signing, migration, or a new dependency, then passed the full local gate, independent review, and CI before PR #239 squash-merged as `2d273b8`. | pass |
| Phase 5 recovery evidence | Candidate `d536a1e` passed its full gate but review found the publish-before-SQLite-commit replay gap. Fresh Phase 5A added exact final-target adoption at both owners without a new schema or abstraction, then passed 2,577 categorized tests, full validation, independent review, and CI before PR #240 squash-merged as `5116f18`. | pass |
...
| No unresolved blocker | Loom has no repository-native way to deliver an opaque assignment capability only to the allocated SLURM job outside scripts, arguments, scheduler metadata, and environment. A supported site-owned delivery channel must be selected before a fresh recovery phase can be approved. | blocked |

Gate result: blocked at the Phase 7 recovery boundary. Phase 7 is read-only
evidence; no Phase 7 recovery plan is approved, and Phases 8 and 9 cannot start
until the maintainer selects the site-owned job-private capability channel.
...
| Phase 3 recovery | Preserve blocked Phase 3A-3C as evidence. Fresh Phase 3D selectively reused validated source/tests, added complete healthy-axis evidence, fail-closed missing/substituted-store handling, stable root binding, startup cleanup, and the full production E2E, then merged through PR #237. | Phase 3C exhausted correction 3/3 after independently reviewed residual failures; the fresh bounded phase preserved explicit findings, budget, review, and merge evidence. | A future disaster-recovery requirement; never an execution compatibility requirement. |
| Phase 4 recovery and retirement proof | Preserve blocked Phase 4/PR #238 as evidence. Fresh Phase 4A selectively reuses its validated source/tests, gives each registration one journal-held random retirement secret and coordinator-held SHA-256 verifier, checks the revealed secret before mutation, and keys polls by principal plus caller poll ID. Each later session receives a new secret. | mTLS proves the current credential, not possession of the original durable journal. A one-time preimage closes that exact gap without durable coordinator secrets, general message signing, or a new dependency; the composite key matches the existing principal-scoped protocol identity. | A broader application-signing requirement or an external fencing authority. |
...
| Clean session rollover | Allow only cooperative old-session retirement after exact empty-set reconciliation and proof of the fresh per-session secret held by the original journal; otherwise use guarded replacement. The coordinator stores only the verifier and checks it before mutation. | A new connection, credential, copied body, or empty replacement database cannot prove the old journal participated. Revealing a one-use old-session secret cannot weaken the next session. | A stronger external agent fencing authority or a general message-signing requirement. |
...
| Ready-stage SLURM route | Retain the explicit per-stage named-profile design and blocked candidate evidence; do not merge or stack it until the allocated job receives one assignment-scoped capability through a selected site-owned job-private channel. Preserve whole-run delegation unchanged. | Profile mTLS alone is too broad, while scripts, arguments, scheduler metadata, environment, and the post-authentication relay cannot securely bootstrap the secret. | Maintainer selects a supported scheduler prolog/container secret file, credential broker, or equivalent delivery boundary; then shape a fresh hard-cut recovery. |
