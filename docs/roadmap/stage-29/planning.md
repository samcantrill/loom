...
Status: maintainer approved; Phase 3C recovery plan ready
Roadmap stage: 29
Evidence baseline: repository source at
`b045f45c763568d8d8cd3e2fbb1e5a8bf80ddf43` plus isolated blocked Phase 3A
evidence `51ca432`/`9d2d7a0` and Phase 3B candidate/review evidence
`a1dfe92`/`da89ff4`; the control checkout was clean when this amendment began
Planning route: the original expanded Stage 29 design remains authoritative.
The Phase 3C amendment is manager-local because independent review already
identified five concrete failures and the maintainer locked the fresh-only
hard-cutover resolution; no product, public, or compatibility decision remains
open.
Current gate: the maintainer approved one fresh bounded Phase 3C from current
`develop`. It may selectively reuse Phase 3B but must close exact runtime
reconstruction, singleton scoped authority ownership, restart capacity,
terminal cancellation, and honest status before the persistent daemon merges.
Blockers: none; commit this planning amendment, then prepare Phase 3C from
current clean `origin/develop` without basing or stacking on Phase 3A/3B
...
| Phase 3 execution evidence | Phase 3A never reached a production worker. Phase 3B candidate `a1dfe92` did reach the real Phase 1/2 path and passed 2,506 categorized tests plus CI, but required review found five reachable contract failures: lossy runtime reconstruction, competing authority owners, full-capacity restart before retained-claim reconciliation, terminal-to-`CANCELLING` regression, and status that hides owner failure. | Phase 3B correction 3/3 is exhausted and PR #235 closed without merge. | Preserve both candidates as read-only evidence; Phase 3C starts from `develop` and closes only the five findings plus safe diagnostics. |
| Approval | The maintainer approved the behavior, refined design, hard managed-local cut-over, and the Phase 3C recommendation: exact fresh-only runtime state, one authority owner, reconcile-before-offer restart, terminal-safe cancellation, and owner-labelled honest status. | None. | Prepare Phase 3C from current `origin/develop`; do not base or stack it on Phase 3A/3B. |
...
| 3C — local-daemon authoritative cut-over | Freshly prepared runs submit, execute, restart, cancel, and report status through the real persistent single-machine daemon while retaining exact execution inputs and conservative cross-owner truth. | Selectively reuse Phase 3B's daemon/client/Phase 1/2 trace; add one protected exact runtime record and digest, singleton authority binding behind a scoped adapter, reconcile-before-offer startup barrier, terminal-safe cancellation projection, owner-available/fresh status, and stable safe diagnostics. No migration/compatibility, remote agent protocol, disconnected control completion, active-process adoption, or positive-containment recovery. | Phase 2 plus read-only Phase 3A/3B evidence. | Exact resources/settings/concurrency round-trip; two-root authority conflict and wrong-owner cancel; retained-claim restart capacity barrier; every terminal-before-cancel race and wait; owner-store failure/status freshness/redaction; real `preprocess -> train`; hard-cutover/delegated-Slurm/import regressions; full gates and independent review. | pending |
| 4 — authenticated agent sessions | Outbound agents on `machine-A` and `machine-B` authenticate, register/reconcile, publish bounded offers, and request work through a no-launch transport gate. | mTLS identity; current-policy role/object/pool authorization per operation/poll; handshake; coordinator-issued persisted session identity versus process/connection epochs; cooperative complete-reference clean session retirement and tombstones; current-epoch fresh offer/work envelopes after coordinator restart; digest idempotency and indeterminate transport outcomes; limits, audit, long-poll ownership; opt-in connectivity receipt. No assignment delivery, artifact bytes, or remote process launch. | Phase 3C. | Direct/HTTP conformance and negative threat/outcome matrix; credential removal on live connection; session-allocation replay/persist-before-offer; reconnect/current-versus-stale epoch and retained-offer rejection, complete clean-rollover/refusal, offer revision, and old-session tombstone tests; opt-in two-machine no-mutation receipt. | pending |
...
Nine numbered phases plus Phase 3B and Phase 3C recovery subphases are an
...
failures. Phase 3C is not another correction or stacked PR. It starts from
current `develop`, treats both isolated candidates as read-only evidence, and
owns only closure of those five findings plus the already accepted Phase 3
vertical outcome in one independently reviewed PR before Phase 4 begins.
...
| Phase 3 recovery evidence | Phase 3A lacked a production execution trace. Phase 3B supplied that trace and passed local/CI gates, then independent review identified five reachable failures against existing FR-3/5/9/11/13/14/19/25 contracts. Phase 3C adds only their smallest fresh-only fixes and focused missing coverage; it does not reopen compatibility or later remote/recovery scope. | pass |
| No unresolved blocker | The five findings and smallest remedies are explicit, no compatibility consumer remains, and the maintainer approved the fresh Phase 3C hard-cutover approach. | pass |

Gate result: passed and maintainer approved for the Phase 3C recovery amendment.
...
| Phase 3 recovery | Preserve blocked Phase 3A/3B as evidence and add one Phase 3C branch/PR from current `develop`. Phase 3C may selectively reuse candidate implementation but owns the exact-runtime, singleton/scoped-authority, restart-capacity, terminal-cancellation, and honest-status fixes plus the full production E2E. | Each blocked phase exhausted correction 3/3; relabelling another correction or stacking would bypass workflow evidence. One fresh bounded phase has explicit accepted findings, budget, review, and merge evidence. | Phase 3C merge or a newly approved narrowing of the persistent-daemon outcome. |
