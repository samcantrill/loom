...
Status: maintainer approved; Phases 1-3D merged; Phase 4 blocked evidence;
Phase 4A pending with its recovery design maintainer-approved
Roadmap stage: 29
...
and squash merge `6a8cf9f`, plus blocked Phase 4 validated/review evidence
`c373d04`/`a991ced` and blocked metadata `e22500f`
Planning route: the original expanded Stage 29 design remains authoritative.
The Phase 4A recovery remains expanded because it repairs a demonstrated trust
boundary. The maintainer approved one fresh per-session retirement secret,
principal-scoped poll identity, and a hard cut-over from the unmerged candidate;
no product, public, or compatibility decision remains open.
Current gate: Phase 4 candidate `c373d04` passed local validation and PR #238
passed CI, but required independent review found forgeable clean-retirement
evidence and globally keyed poll identity after correction 3/3 was exhausted.
PR #238 closed without merge. Fresh Phase 4A is the next pending phase with an
approved design.
Blockers: none
...
| Phase 3 execution evidence | Phase 3A never reached a production worker. Phase 3B reached the real Phase 1/2 path but failed five contracts. Phase 3C closed those five but review found two residual failures. Fresh Phase 3D closed complete owner evidence, missing/substituted-store handling, and partial-start cleanup, then passed 2,538 categorized tests, `make validate-pr`, independent review after correction 2/3, and CI. | None. PR #237 squash-merged as `6a8cf9f`; Phase 3A-3C remain read-only blocked evidence. | Preserve Phase 3D as the current base for the fresh Phase 4A recovery. |
| Phase 4 execution evidence | Candidate `c373d04` establishes the complete no-launch mTLS/session/offer/poll path and passed 2,554 categorized tests plus CI. Review proved that any credential holder could invent the syntactically valid agent-side retirement digest, and that two principals choosing the same poll ID collide in SQLite. | Correction 3/3 is exhausted; PR #238 is closed and the branch is read-only evidence. | Start fresh Phase 4A from current `develop`, selectively reuse the validated source/tests, and close only these two findings. |
| Approval | The maintainer approved the hard cut-over: each registration journals a fresh one-session retirement secret before send, the coordinator stores only its verifier, retirement requires that secret before any mutation, and poll identity is `(principal_id, poll_id)`. No candidate-schema compatibility or migration is required. | None. | Record the exact recovery boundary, independently check the trust design, then implement Phase 4A. |
...
Nine numbered phases plus Phase 3B, Phase 3C, Phase 3D, and Phase 4A recovery subphases are an
...
reviewed PR before Phase 5 begins.
...
| Phase 4 recovery evidence | Candidate `c373d04` passed the full local gate and PR #238 passed CI. Review demonstrated a forgeable retirement proof and global poll key; the approved Phase 4A design gives journal possession and poll isolation one authoritative owner each without general signing, migration, or a new dependency. One removal-first safety review passed; one plan review found only an ambiguous `approved` lifecycle label, corrected to `pending` with design approval explicit. | pass |
| No unresolved blocker | The maintainer approved the fresh-only per-session secret/verifier and composite poll-key resolution. Phase 4A has no remaining product, durable-format, compatibility, or trust decision. | pass |

Gate result: passed; Phase 4 is blocked read-only evidence and Phase 4A is the
next approved phase.
