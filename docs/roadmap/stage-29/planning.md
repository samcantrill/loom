...
Roadmap stage: 29
Evidence baseline: source at `e2712bee936ac88110c8b05ead475b93b842da76`;
subsequent Stage 29 commits are documentation-only, and startup review
reconfirmed the cited source seams against current `develop`
...
| FR-15 | A definitive failed/cancelled attempt uses existing reliability policy to decide the next attempt, which may be placed elsewhere. Accepted but unreachable work is unknown and is never automatically retried or reassigned. Manual recovery intent freezes ordinary mutation but continues to durably retain exact-current-fence terminal facts. Immediately before close, any complete reachable success/result is finalized/committed or blocks recovery; authority success commit and manual close compete on the same expected execution fence. Success winning aborts recovery, while close winning makes only later facts stale. Known committable success is not discarded. | Timeout and process absence do not prove failure; an unobservable result on an unavailable machine remains explicit operator risk. | Retry/outage plus retained-before/during/after-close success-recovery races. | locked |
| FR-16 | Granted stages continue while the coordinator or lifecycle authority is temporarily unavailable. Agents durably journal events, reconnect, reconcile, replay, and publish a fresh offer. Coordinator outage prevents new/downstream assignments; authority outage additionally makes the coordinator pause preparation, binding, grant, terminal commit, and new delivery. Neither stops already-granted work. After authority restart, a new service generation is accepted only through authenticated reconciliation with the configured workspace and a complete retained-run continuity set: every coordinator-retained admitted run is present with its last-acknowledged authority revision and canonical full-snapshot fingerprint, and every nonterminal attempt/execution fence matches exactly. A pristine empty authority is valid only when the coordinator has no retained admitted runs; otherwise missing or inconsistent truth fails closed. | Coordinator HA and automatic authority replacement are deferred. | Coordinator/authority restart, disconnect, first-bootstrap, retained-run continuity, and mismatch tests. | locked |
| FR-17 | Persistent HTTP peers use mTLS and scoped principals. Owner-contained local IPC may instead use verified operating-system peer identity. Direct composition invokes the same authorizer. Every cross-process coordinator-to-authority call authenticates both roles and authorizes a least-privilege coordinator principal; agents and workers receive neither authority credentials nor direct authority access. Assignment/grant messages bind coordinator generation, agent session, stage work, claims, nonces, and idempotency keys. | Loopback/network location is not authentication, and authenticated payloads cannot select code, paths, credentials, or providers. | Authentication/authorization/replay and authority-isolation tests. | locked |
...
| FR-25 | The coordinator and authority boundaries have an explicit threat model. Every remote operation uses authenticated transport, connection-derived principal identity, per-operation role/object/pool scopes, expected versions, idempotency scoped to principal and request digest, strict schema/content-type/size/cardinality limits, and bounded redacted errors/audit facts. Coordinator-to-authority operations additionally verify authority service/workspace/generation identity and use only coordinator lifecycle scopes. Artifact operations use coordinator-issued assignment-scoped transfer identities and derived safe storage locations, never caller-selected host paths or arbitrary fetch URLs. | mTLS or peer credentials authenticate transport identity; neither supplies authorization, replay prevention, sandboxing, at-rest encryption, or permission for a body-supplied actor. Hosted multi-tenant identity federation and hostile-code isolation are deferred. | Direct/HTTP scope matrix, authority service/principal mismatch, body/URL identity mismatch, replay/different-body conflict, downgrade/oversize, traversal/symlink/SSRF, quota, and redaction tests. | locked |
...
| FQ-3 | FR-5–FR-7 | Resources and preferences are stage-specific; run-level placement policy supplies defaults, a pool, and optional hard pinning, while separate orchestrator policy supplies concurrency. | Training preferences no longer distort preprocessing/evaluation placement, and `max_parallel_stages` does not enter placement identity. | More explicit configuration. | locked |
...
Run-level `max_parallel_stages` remains a separate orchestrator limit over
exposed/active attempts. It is not a resource quantity, candidate constraint,
preference, or input to the resolved-placement fingerprint; changing the limit
changes admission concurrency without changing what one stage needs.
...
| Impersonated or stale authority supplies lifecycle truth | The coordinator verifies the authority service, workspace, generation, schema, and required capabilities before readiness or mutation. A changed generation is adopted only after complete retained-run continuity is proven and durably recorded: every retained run reproduces its last-acknowledged revision/canonical full-snapshot fingerprint and nonterminal attempt/fence state matches exactly. A pristine empty authority is accepted only beside a coordinator with no retained admitted runs; mismatch fails closed and publishes no work. |
...
workspace/schema/capabilities, and reads a complete continuity snapshot for
every coordinator-retained admitted run. Durable run identity and terminal
summary must remain present; each nonterminal attempt and execution fence must
match exactly. One coordinator transaction then records the new accepted
authority generation before work resumes. Old-generation requests are rejected.
A pristine empty authority is allowed only when the coordinator has no retained
admitted runs. The coordinator stores each accepted authority revision and a
canonical fingerprint of the corresponding complete authority snapshot solely
as continuity evidence; that checkpoint is not lifecycle truth and cannot
repopulate authority. Missing, divergent, or inaccessible expected truth leaves
the system degraded; it never attaches to a replacement service or reconstructs
lifecycle truth from coordinator projections.
...
- A durable coordinator `RunOrchestrator` invokes that predicate, prepares
  ...
  derives terminal run/queue state. Preparation is one authority expected-state
...
materialization and the complete local assignment/grant/launch saga; Phase 3 owns the
...
| `ManagedLocalQueueRuntime.resolve_recovery_unknown(...previous_processes_confirmed_stopped=True...)` | Quarantine from all Stage 29 state; retain only for exact historical whole-run records if compatibility still requires it. | A body/API boolean is not trusted positive-containment evidence. Phase 8 owns the only Stage 29 fence/close/retry operation. |
...
| DQ-3 | FR-5–FR-7 | Resource-specific planners resolve and claim through scheduling-owned validated views; pipeline runtime owns adaptation to and from the existing resource codec; core orders candidates. | Avoids a universal resource schema and a `scheduling -> pipeline -> runtime -> scheduling` import cycle while keeping atomicity central. | Explicit higher-level composition is required. | locked |
...
| DQ-14 | FR-9, FR-16, FR-17, FR-25 | Preserve the authority service as a separate owner and expose one authenticated least-privilege coordinator view; never give agents/workers direct authority access. Reconcile a rotated service generation only after complete retained-run authority/coordinator continuity; allow pristine-empty bootstrap only when both sides have no retained admitted run. | Closes a mutation bypass and terminal-history loss while retaining the established no-direct-database architecture, one lifecycle owner, and durable authority restart. | Local deployment needs an owner-contained authenticated adapter; persistent HTTP needs service identity/mTLS, and outage recovery needs one explicit generation hand-off. | locked |
...
| Coordinator security stopped at the application edge | FR-9, FR-16, FR-17, FR-25; DQ-14 | The current lifecycle authority is a distinct restartable service, so an unauthenticated loopback/client path would bypass the coordinator authorizer, while blindly accepting its rotated generation could attach Stage 29 to divergent truth. Checking only nonterminal attempts could also lose retained terminal history, while rejecting every empty authority would prevent first bootstrap. | Keep authority service ownership, add an authenticated scoped coordinator principal, reconcile a new generation against the complete retained-run continuity set, allow pristine-empty bootstrap only when the coordinator is also empty, and exclude authority access from agents/workers. | corrected in deep startup review |
...
| Coordinator or authority disconnect | Granted stage completes and buffers/replays; no downstream stage starts until coordinator and authority are usable. A rotated authority generation resumes only after complete retained-run continuity; missing/divergent expected truth remains degraded, while pristine-empty bootstrap is valid only when the coordinator has no retained admitted run. | Agent journal + coordinator/authority generation reconciler. | Real process interruption plus first-bootstrap and authority restart/negative continuity matrix. | planned |
...
| Cancellation versus grant/start/success | Cancel-first prevents grant. A granted assignment with no start intent needs exact never-launched acknowledgement; durable start intent with unknown outcome stays unknown; confirmed start requires containment; committed success remains truthful. | Coordinator transaction + agent start journal/process owner + authority. | Barriers before/after grant, start intent, launcher outcome/event, and terminal commit. | planned |
...
| Remote threat matrix | Wrong role/object/pool, body actor, authority service/principal/workspace/generation, changed idempotent body, old version/fence, oversized payload, arbitrary URL/path, traversal/symlink, and raw-error attempts fail before unsafe mutation/access. | TLS/peer-identity edges + application and authority authorizers/codecs + transfer adapter. | Direct/IPC/HTTP/authority/artifact negative matrix. | planned |
...
| --- | --- | --- | --- | --- | --- |
| 1 — scheduling kernel and ready-stage work | An admitted run is reconciled into authoritative dependency-ready exact `PENDING` attempts and rebuildable stage work, and Loom calculates deterministic, explainable placement against immutable local snapshots without reserving capacity or launching. | Placement resolution; CPU/memory planners; component descriptors, frozen registries, conformance; fixed pure kernel; shared readiness predicate; idempotent authority attempt preparation; controller-action reconciliation; rebuildable stage-work projection. No assignment, resource claim/provider mutation, agent journal, worker materialization, daemon, or process launch. | Implemented planner, authority state, `ResourceRequest`, and Stage 25/28 extension patterns. | Pure/default/custom component tests; train/evaluate and diamond readiness; concurrent/replayed preparation without duplicate attempts; restart/rebuild; mutation sentinels and no-launch assertion. | pending |
| 2 — durable local stage execution | A bounded local run executes ready stages through the final reservation, authority bind, physical prepare, grant, launch, result, commit, and release path. | Coordinator assignment/reservation operations; authority CAS/fence; local agent journal and `AgentResourceProvider`; composite CPU/memory admission; local artifact hand-off; execution-only worker adaptation with no managed whole-run lock. No persistent daemon or remote protocol. | Phase 1. | Two-stage and diamond local E2E with real branch overlap; exhaustive crash/decline/activation/one-launch/release matrix. | pending |
| 3 — persistent local daemon | Multiple clients submit, monitor, and conservatively cancel runs through a persistent single-machine coordinator/agent composition; existing managed facades use the same stage path. | Scoped local application views and authorizer; authenticated least-privilege coordinator-to-authority view with fail-closed generation reconciliation; SQLite role roots/locks; daemon/client lifetime; queue/API/CLI compatibility; fail-closed ordinary restart/status; connected local cancellation; legacy boolean recovery quarantine. No active-process adoption or remote agents; persistent HTTP, if chosen locally, already requires mTLS. | Phase 2. | Multi-run daemon E2E; authority outage/restart/identity/scope/continuity isolation; duplicate start, zero-availability/unknown restart, cancellation, old-record, legacy-recovery isolation, facade, and redaction tests. | pending |
...
| 5 — remote CPU/memory stage execution | Ready CPU/memory stages execute on an authenticated remote agent, with durable regular-file inputs, coordinator-mediated artifact transfer, result replay, and coordinator/authority-outage continuation. | Cross-agent/cross-pool CPU/memory availability; remote assignment delivery; regular-file relay and safe staging; remote agent loop; grant/start; result/output finalization; reconnect and zero-availability reconciliation. No directory/tree relay, GPU placement, or automatic unknown-work failover. | Phase 4. | `machine-A`/`machine-B` CPU/memory E2E; payload-capability/local-fallback and transfer security/faults; coordinator/authority disconnect/restart barriers; no duplicate launch. | pending |
...
| 8 — restart and guarded recovery | Agents restart without duplicate launch, and privileged operators can resolve positively contained unknown work or replace a fully contained old session. | Same-session journal/process/outbox recovery; user-service operation; positive-containment evidence; cross-store fence/close/retry reconciliation; stale-event rejection; complete-set session replacement; regression only for Phase 5 coordinator/authority restart. No second restart state machine, automatic failover, or coordinator HA. | Phase 7. | Restart at every process/output edge; coordinator/authority restart regression; weak-evidence rejection; idempotent recovery; known-success and stale-output races; full Stage 29 validation. | pending |
...
introducing an unused data-plane API.
...
three durable owners, two authenticated service boundaries, cross-store crash
reconciliation, remote artifact movement, and privileged recovery across eight
...
| Contracts and private discretion clear | Identity, store ownership, correctness kernel, extension authority, hand-off, resource resolution, application/authority authorization, artifact access, and compatibility are fixed; table/route/helper layout remains private. | pass |
...
| Phases vertical and reviewable | The approved eight-phase exception isolates kernel/readiness plus idempotent `PENDING` preparation, local execution side effects, daemon lifetime, remote trust, remote data/execution, GPU placement, ordinary controls, and privileged recovery. The foundation phase adds only its necessary `PENDING` authority preparation and rebuildable coordinator projection while reusing existing controller-action authority transitions; indivisible execution sagas remain intact. | pass |
