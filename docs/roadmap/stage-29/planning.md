...
Status: maintainer approved; nine-phase implementation plan ready
Roadmap stage: 29
Evidence baseline: repository source at
`bdaf1e06f24c8e023612fc0794ce8d454b862342`; relevant dirty paths are the
current Stage 29/feature/roadmap/structure documentation refinements, with no
source or test changes during this planning pass
Planning route: expanded because the amendment changes the managed execution
unit from a whole run to an individual stage attempt and the later passes add
subsystem-public extension, durable implementation-identity, trust,
data-transfer, lifecycle, search-completeness, preference-policy, and external
SLURM submit/bootstrap/recovery boundaries
Current gate: maintainer approved the per-stage direction, generic interface/
security refinement, explicit per-stage ready-work SLURM delegation,
nine-phase implementation shape, and deep scheduler
correctness refinements; the manager-local cross-component correctness review
has also fixed admission, epoch, replay, cancellation, recovery, status, and
durable-store boundaries, including explicit initialization, pending-cancel
ordering, session-allocation replay, and accepted-time fencing
...
| Approval | Maintainer approved the behavior, refined design, explicit per-stage SLURM route, nine-phase implementation manifest, deep scheduler corrections, and clarified deployment/startup boundaries. | None. | Prepare Phase 1 from current `origin/develop`; do not start a later phase early. |
...
| Queue admission schema | Current SQLite uniqueness is on `queue_item_id`; `run_uri` is indexed data but is not a unique managed admission key. | Add an immutable managed-admission identity/digest and preserve historical duplicates only through compatibility reads. | FR-3, FR-18, FR-25 |
...
- Public/durable impact: runtime placement options, normalized stage placement
  ...
  compatibility behavior for existing whole-run queue records.
...
| FR-18 | Existing queue records and public managed facades remain readable/callable through explicit compatibility adapters and schema migration. Whole-run `argv`/resource dispatch is deprecated only for managed execution. Existing SLURM single-job and `afterok` whole-run modes, queue identities, manifests, and public operations retain their historical ownership and behavior; the new ready-stage route uses separate identities and records rather than migrating them in place. | Do not reinterpret historical durable records, route a historical delegated run through the managed orchestrator, or use the `afterok` DAG as Stage 29 readiness. | Old-record/API, existing whole-run SLURM, and new-record separation tests. | locked |
...
| DQ-9 | FR-18, FR-27 | Compatibility-wrap managed whole-run APIs and preserve historical whole-run SLURM delegation unchanged; give the new ready-stage route separate managed assignment/submission identities. | Limits breakage and prevents existing `afterok` or queue manifests from becoming a second Stage 29 DAG/lifecycle owner. | Two explicit SLURM paths coexist until a later measured migration. | locked |
...
Nine phases are an explicit exception to the normal one-to-three preference.
...
Gate result: passed and maintainer approved. The previous expanded design,
...
SLURM scope/correctness findings are closed. No planning blocker remains.
