...
Status: maintainer approved; Phase 3B recovery plan ready
Roadmap stage: 29
Evidence baseline: repository source at
`66d49684a35b9ed2ce1461b27596579b97eb6ffb` plus the isolated blocked Phase 3
candidate at `9d2d7a0` (`51ca432` implementation state); the control checkout
was clean when this recovery planning pass began
Planning route: expanded because the amendment changes the managed execution
unit from a whole run to an individual stage attempt and the Phase 3 recovery
crosses the coordinator, authority, agent, public-facade, and durable-root
boundaries. The original expanded design remains authoritative.
Current gate: the maintainer approved the reviewed recovery artifacts: preserve the hard
cut-over, keep the blocked candidate isolated, and add one fresh bounded Phase
3B whose production composition must prove the existing Phase 1 orchestrator
and Phase 2 execution saga through a real daemon/client path. Expanded design
and plan reviews passed after one bounded artifact correction.
Blockers: none; Phase 3B may prepare from current clean `origin/develop` after
this planning amendment is committed
...
| Phase 3 execution evidence | The isolated candidate proves owner-private IPC, fresh-root/schema rejection, digest-bound admission, and a typed Phase 2 hand-off, but its example uses supplied recording fakes. No production owner constructs `RunOrchestrator`, resolves/reserves a real assignment, or invokes the Phase 2 saga after socket admission. | The accepted daemon outcome remains blocked; green component tests do not prove a reachable user execution path. | Reuse the candidate only as implementation evidence inside Phase 3B and require a real `preprocess -> train` trace before submission. |
| Approval | The maintainer approved the behavior, refined design, hard managed-local cut-over, and reviewed Phase 3B recovery artifacts; expanded design and plan reviews passed after one bounded artifact correction. | None. | Prepare Phase 3B from current `origin/develop`; do not base or stack it on Phase 3A. |
...
| Managed local runtime | Public facade composes the current queue/controller/local process adapter. | Remove the whole-run managed-local runtime and replace it with prepared-run daemon/client operations. No adapter or warning-only fallback is retained. | FR-1, FR-18 |
...
| FR-3 | A queue item and `run_uri` remain the submission/control identities. New admission is unique on `(coordinator_id, run_uri)`, where the durable root's stable coordinator ID is the namespace, and pins an immutable normalized intent digest plus one run execution owner: managed-stage, delegated-whole-run, or historical compatibility. Exact replay returns the same admission; changed intent/owner conflicts; resume targets that admission, while a rerun needs a new `run_uri`. A coordinator commit may return a durable `PENDING_AUTHORITY` admission while authority is unavailable, but only an exact authority owner/intent/operation-receipt reconciliation makes it `ACTIVE` and schedulable; conflict is a visible blocked state, never fallback to another owner. Within a managed-stage run, each exact stage assignment separately pins a `managed_agent` or `slurm` target; this does not transfer whole-run ownership. Managed scheduling uses `(run_uri, stage_name, attempt, readiness_generation)` plus distinct stable `stage_work_id`, `assignment_id`, `slurm_submission_operation_id` when applicable, and `process_execution_id`. Rebuilding a projection reproduces its `stage_work_id`; once referenced, it is never re-keyed. | Never overload “job” or queue item as the execution-attempt identity, admit the same run to two run owners, let one attempt receive two target assignments, schedule before authority ownership is proven, or interpret a submit timeout as absence. | Admission uniqueness/digest/replay, managed-run/mixed-target ownership, pending-owner/owner-conflict, stable-rebuild identity, and codec tests. | locked |
...
| FR-9 | The coordinator persists immutable run admissions, stable coordinator ownership, rebuildable but identity-stable stage-work projections, and durable assignment/claim/control/event facts; the per-run authority remains the sole owner of plans, cancellation epoch, attempts, stage/run status, inputs, output commits, and retry facts. Each managed authority run is bound to one stable `coordinator_id`; a process epoch may rotate only for that same durable owner after reconciliation. | No database may silently overwrite another owner's truth. A second coordinator identity, embedded runtime, or delegated path cannot attach to the same managed run. Copied-state concurrent coordinators remain unsupported split brain. | Ownership, competing-coordinator/entrypoint, projection-rebuild, and restart tests. | locked |
...
| FR-11 | Agents publish versioned, expiring inventory and availability, then perform final local admission/binding against current truth. Expiry uses coordinator-accepted receipt time. Coordinator restart/process-epoch change requires session reconciliation and a freshly received current-epoch offer/work request before new delivery; a retained old offer cannot create a new assignment. A stale offer may be declined without starting the attempt. At most one admission remains unresolved per availability revision, but once an accepted claim is reflected in a fresh net revision the same agent may run another disjoint assignment on remaining atoms. | Coordinator reservations do not prove physical acquisition. Admission serialization must neither reuse one stale snapshot nor reduce an agent to one active stage. | Offer/bind drift, coordinator restart/fresh-offer, same-agent disjoint overlap, and same-atom exclusion tests. | locked |
...
| FR-13 | Each run honors `max_parallel_stages`; semantic readiness and rebuildable stage-work projection do not consume a slot. The coordinator assignment transaction atomically rejects a new reservation when the run already has the configured number of managed-agent or SLURM assignments in reserved, bound, submitting, submission-unknown, externally queued, accepted, granted, running, or other unknown active states. Independent ready branches may otherwise run concurrently and work from other runs may fill capacity. | An unassigned `PENDING` attempt is visible to scheduling but does not count. A SLURM `PENDING` job holds no agent claim but still occupies a run concurrency slot. A run lock cannot remain held by one in-memory loop for the full managed run. | Concurrent mixed-route final-slot reservation, SLURM pending/unknown counting, blocked-first-branch, terminal release/restart, and parallel execution tests. | locked |
| FR-14 | A client cancellation request is first committed durably by the coordinator, then installed by authority CAS as the one canonical run cancellation intent/epoch. Only the effective authority intent stops readiness, bind, grant, descendant creation, and retry; coordinator fan-out projects it into exact assignment controls. A request already present on `PENDING_AUTHORITY` admission is installed after owner binding but before `ACTIVE` promotion/work exposure. Status distinguishes requested, effective, settling, and terminal cancellation. Finalization waits for authoritative terminal truth or positive-containment evidence, and a concurrent valid terminal stage fact remains truthful. | Connectivity loss, a sent signal, or a coordinator-only flag is not cancellation completion or cancellation truth. The pending-admission ordering does not give the coordinator request authority semantics. | Request/pending-admission/authority-outage replay and cancel/readiness/bind/grant/start/success race tests. | locked |
...
| FR-18 | Managed-local whole-run facades, requests, and roots are removed under the approved hard cut-over; the replacement accepts only stage-based prepared-run operations against fresh daemon roots. Existing generic/delegated queue ownership and SLURM single-job/`afterok` whole-run modes, identities, manifests, and public operations retain their historical behavior; the new ready-stage route uses separate managed identities and records. | Do not translate an old managed-local row into missing plan, attempt, placement, claim, fence, or owner truth; route a historical delegated run through the managed orchestrator; or use the `afterok` DAG as Stage 29 readiness. | Old managed-local import/request/root rejection, existing whole-run SLURM, and new-record separation tests. | locked |
| FR-19 | Status joins but never flattens owner facts. It exposes separately versioned admission/control, authority lifecycle/cancellation, scheduling/route, assignment/execution, SLURM dispatch/external scheduler, transfer/result, and service-health axes with coordinator-accepted receipt time and freshness. The join is not a globally atomic snapshot: each axis carries its owner revision, and the top-level `as_of` is the coordinator read boundary. Remote wall clocks never decide ordering, expiry, or freshness. Authority terminal state remains lifecycle truth; coordinator, agent, bootstrap, and SLURM axes may explain waiting or unknown work but cannot overwrite it. An unavailable owner or coordinator-clock anomaly yields labelled stale/degraded evidence, never inferred terminality, successful output, or resource release. | Snapshot-relative diagnostics and a top-level display summary are not new durable truth. SLURM `COMPLETED` without a current-fence validated Loom result is not stage success, and operational unknown is not silently added to authority `StageStatus`. | Multi-owner/SLURM skew, completed-without-result, coordinator rollback/jump, precedence, staleness, and redaction tests. | locked |
...
- Phase 3B adds one production application composition above queue transport and
  ...
  resolves the canonical persisted execution plan and runtime metadata from
  their owning store, and pins the normalized admission digest. It obtains the
...
| `ManagedLocalQueueRuntime` | Remove. | The supported replacement is the persisted-plan persistent daemon/client composition; there is no callable compatibility wrapper. |
...
| `continue_prepared_run(whole_run)` | Remove from managed-local compatibility. | The new daemon resolves the canonical persisted execution plan and runtime metadata by the admitted `run_uri` and pins their normalized intent digest; it does not translate the old whole-run continuation request or add another prepared-run identity. |
...
| DQ-9 | FR-18, FR-27 | Remove managed-local whole-run APIs and reject their roots under the approved hard cut-over; preserve historical whole-run SLURM and generic/delegated queue ownership unchanged, and give the new ready-stage route separate managed assignment/submission identities. | Old managed-local state cannot supply the new plan, attempt, placement, claim, fence, and owner facts without fabrication, while existing delegated SLURM remains a valid separate owner. | Managed-local callers must prepare new runs and initialize fresh daemon roots; two explicit SLURM paths continue to coexist. | locked |
...
| Cancellation versus authority/grant/start/terminal | Coordinator request survives authority outage but is labelled only requested; effective authority epoch prevents readiness/bind/grant/descendant/retry. A granted assignment with no start intent needs exact never-launched acknowledgement; durable start intent with unknown outcome stays unknown; confirmed start requires containment; terminal facts remain truthful. | Coordinator cancellation request + authority epoch/CAS + agent start journal/process owner. | Barriers before/after request, authority intent, readiness, bind, grant, start intent, launcher outcome/event, and each terminal commit. | planned |
| Joined status during partial outage/clock skew | Every owner axis retains its revision/coordinator-accepted receipt time/freshness; a non-atomic join labels `as_of`, and an unavailable owner yields stale/degraded evidence without overwriting authority terminal state or implying release. | Joined status projector. | Fixed-precedence, skewed remote clocks, interleaved owner mutation, and machine-output tests across authority/coordinator/agent/transfer outages. | planned |
...
| 3B — local-daemon production composition | Multiple clients submit, monitor, wait, and conservatively cancel immutable prepared runs through one persistent single-machine coordinator/agent composition that automatically drives Phase 1 readiness and Phase 2 execution. | Protected production authority/run-store/orchestrator/scheduler/offer/reservation/agent composition; daemon wake/reconcile loop; stage-based Python/socket/CLI operations; joined status; fresh-root hard cut-over; retained stable identity and restart/cancellation gates. No legacy adapter/migration, remote agent protocol, active-process adoption, or Phase 8/9 controls/recovery. | Phase 2 plus selectively reused Phase 3A implementation evidence. | Real `preprocess -> train` daemon E2E with no injected fakes; multi-run capacity; pending-authority recovery; restart/no-duplicate hand-off; connected cancellation; owner-labelled status; fresh-root/schema/permission/lock checks; old managed-local rejection; delegated Slurm regression; package/import direction. | pending |
| 4 — authenticated agent sessions | Outbound agents on `machine-A` and `machine-B` authenticate, register/reconcile, publish bounded offers, and request work through a no-launch transport gate. | mTLS identity; current-policy role/object/pool authorization per operation/poll; handshake; coordinator-issued persisted session identity versus process/connection epochs; cooperative complete-reference clean session retirement and tombstones; current-epoch fresh offer/work envelopes after coordinator restart; digest idempotency and indeterminate transport outcomes; limits, audit, long-poll ownership; opt-in connectivity receipt. No assignment delivery, artifact bytes, or remote process launch. | Phase 3B. | Direct/HTTP conformance and negative threat/outcome matrix; credential removal on live connection; session-allocation replay/persist-before-offer; reconnect/current-versus-stale epoch and retained-offer rejection, complete clean-rollover/refusal, offer revision, and old-session tombstone tests; opt-in two-machine no-mutation receipt. | pending |
...
Nine numbered phases plus the one Phase 3B recovery subphase are an explicit
exception to the normal one-to-three preference. Phase 3B exists because the
Phase 3A implementation exhausted its correction budget and failed its
pre-submit gate; it is not a relabelled fourth correction. The replacement
starts from current `develop`, treats the isolated candidate as read-only
evidence, and must deliver the previously accepted Phase 3 vertical outcome in
one independently reviewable PR before Phase 4 begins.
...
| Phase 3 recovery evidence | The manager pre-submit gate identified one supported-path failure: socket admission has no production authority/orchestrator/reservation/Phase 2 composition. Expanded design review verified the Phase 1/2 seams and candidate gap, aligned FR-18/DQ-9 with the approved hard cut-over, and removed opaque prepared-stage input as a second plan authority. Phase 3B now adds only one private production composition and one real E2E. | pass |
| No unresolved blocker | Product choices are locked, expanded artifact reviews passed, and the maintainer approved the reviewed recovery artifacts. | pass |

Gate result: passed and maintainer approved for the Phase 3 recovery amendment.
...
| Managed-local compatibility | Hard cut-over: resolve only the canonical persisted execution plan and runtime metadata for stage-based submission against fresh daemon roots. Reject old managed-local APIs/imports/requests/roots/records without translation into new admissions, execution, mutation, or deletion; bounded schema identification is allowed only to return an actionable incompatibility. Preserve delegated whole-run Slurm. | The old whole-run records cannot supply authoritative prepared attempt, placement, claim, fence, and owner facts without fabrication, and a caller-supplied mapping cannot replace the persisted plan owner. | A future explicitly designed offline archival tool, not an execution compatibility requirement. |
| Phase 3 recovery | Preserve blocked Phase 3A as evidence and add one Phase 3B branch/PR from current `develop`. Phase 3B may selectively reuse the candidate but owns the full production composition and E2E. | Three correction passes were exhausted and the same missing reachable path cannot be relabelled as another correction. A new bounded phase provides fresh ownership, budget, review, and merge evidence. | Phase 3B merge or a newly approved narrowing of the persistent-daemon outcome. |
