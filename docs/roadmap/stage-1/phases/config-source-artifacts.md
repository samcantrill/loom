...
- Phase execution plan path: `docs/roadmap/stage-1/phases/config-source-artifacts.md`
...
- Phase implementation refinement: used on 2026-05-06 by `loom_phase_refiner`; no product-code or test blocker found.
- Pre-submit blocker gate: used; exact finding was nested local/file includes inside user include replacement targets not being recorded or snapshotted.
- User-authorized blocker-resolution pass: used on 2026-05-06 for the scoped pre-submit blocker only.
- PR body draft: completed on 2026-05-06 by `loom_pr_preparer`; durable draft at `docs/roadmap/stage-1/phases/config-source-artifacts-pr-body.md`.
- PR body refine: used on 2026-05-06 for expanded path; PR opened and verified against the recorded root target branch.
- PR review: unused.
...
- Implementation summary: completed the Phase 15 raw snapshot opt-in by adding the keyword-only `include_raw_source_snapshots` API to public compose and inspection entrypoints, public plain-data `RawSourceSnapshotBundle`/payload/reference helpers, internal loader/include raw-text capture only when opted in, deduped local/file base-overlay-include payloads, recipe `unsupported_source_kind` references, default disabled `not_requested` references, and metadata-only limitation facts on source artifacts, manifest/provenance metadata, and inspection stages without embedding raw content in persisted artifact contracts or fingerprints.
- Implementation validation: targeted Phase 15 package/import-boundary, unit artifact/compose/fingerprint, contract artifact/inspection, and integration provenance/fingerprint/source-snapshot suites passed with 77 tests using `UV_CACHE_DIR=/tmp/loom_uv_cache uv run --extra config pytest tests/package/test_config_api.py tests/package/test_import_boundaries.py tests/unit/loom/config/test_config_artifacts.py tests/unit/loom/config/test_compose.py tests/unit/loom/config/test_config_fingerprints.py tests/contracts/test_config_artifact_contract.py tests/contracts/test_config_composition_inspection_contract.py tests/integration/config/test_compose_provenance.py tests/integration/config/test_compose_fingerprints.py tests/integration/config/test_compose_source_snapshots.py`. Final `UV_CACHE_DIR=/tmp/loom_uv_cache make validate-pr` passed with Ruff, Pyright, default suite 429 passed/10 skipped, config-extra suite 300 passed/434 deselected, and build success. Final `UV_CACHE_DIR=/tmp/loom_uv_cache make test-summary` passed and wrote `build/test-summary.md` with package 36 passed/1 skipped, unit 354 passed/1 skipped, contract 30 passed/2 skipped, integration 9 passed/5 skipped, e2e 5 passed, and config-extra 300 passed/434 deselected.
- Refinement summary: expanded-path implementation refinement pass completed; no product code or test changes were needed; phase notes updated to mark the refinement budget used; no PR opened.
- Refinement validation: targeted Phase 15 suite passed with 77 tests using `UV_CACHE_DIR=/tmp/loom_uv_cache uv run --extra config pytest tests/package/test_config_api.py tests/package/test_import_boundaries.py tests/unit/loom/config/test_config_artifacts.py tests/unit/loom/config/test_compose.py tests/unit/loom/config/test_config_fingerprints.py tests/contracts/test_config_artifact_contract.py tests/contracts/test_config_composition_inspection_contract.py tests/integration/config/test_compose_provenance.py tests/integration/config/test_compose_fingerprints.py tests/integration/config/test_compose_source_snapshots.py`. Final refinement gate `UV_CACHE_DIR=/tmp/loom_uv_cache make validate-pr` passed with Ruff, Pyright, default suite 429 passed/10 skipped, config-extra suite 300 passed/434 deselected, and build success. Final refinement evidence command `UV_CACHE_DIR=/tmp/loom_uv_cache make test-summary` passed and wrote `build/test-summary.md` with package 36 passed/1 skipped, unit 354 passed/1 skipped, contract 30 passed/2 skipped, integration 9 passed/5 skipped, e2e 5 passed, and config-extra 300 passed/434 deselected.
- Pre-submit blocker resolution: fixed the user-authorized scoped blocker by passing `raw_source_texts` through user replacement target include expansion and propagating replacement-target nested include records into effective include sites. The replacement expansion now keeps nested include artifact metadata shifted to the final config path while preserving source-local include paths, so `source_artifacts`, manifest/provenance raw snapshot references, and caller-owned payloads include base, replacement, and nested leaf local/file sources. Added focused integration coverage for a user include replacement whose replacement file includes another local file and verifies resolved nested content, source artifact paths, reconstructable raw snapshot payloads, and manifest/provenance references. No Phase 16, persistence, run-store, CLI, pipeline, remote/plugin/global resolver, secret-policy, or `_copy_` behavior was added.
- Pre-submit blocker validation: focused `UV_CACHE_DIR=/tmp/loom_uv_cache uv run --extra config pytest tests/integration/config/test_compose_source_snapshots.py` passed with 5 tests. Targeted Phase 15 suite passed with 78 tests using `UV_CACHE_DIR=/tmp/loom_uv_cache uv run --extra config pytest tests/package/test_config_api.py tests/package/test_import_boundaries.py tests/unit/loom/config/test_config_artifacts.py tests/unit/loom/config/test_compose.py tests/unit/loom/config/test_config_fingerprints.py tests/contracts/test_config_artifact_contract.py tests/contracts/test_config_composition_inspection_contract.py tests/integration/config/test_compose_provenance.py tests/integration/config/test_compose_fingerprints.py tests/integration/config/test_compose_source_snapshots.py`. Final blocker-resolution gate `UV_CACHE_DIR=/tmp/loom_uv_cache make validate-pr` passed with Ruff, Pyright, default suite 429 passed/10 skipped, config-extra suite 301 passed/434 deselected, and build success. Final blocker-resolution evidence command `UV_CACHE_DIR=/tmp/loom_uv_cache make test-summary` passed and wrote `build/test-summary.md` with package 36 passed/1 skipped, unit 354 passed/1 skipped, contract 30 passed/2 skipped, integration 9 passed/5 skipped, e2e 5 passed, and config-extra 301 passed/434 deselected.
- PR preparation: expanded-path PR body draft completed at `docs/roadmap/stage-1/phases/config-source-artifacts-pr-body.md`; refine/open pass completed on 2026-05-06. Confirmed worktree `/home/samcantrill/work/loom-worktrees/config-source-artifacts`, branch `codex/config-source-artifacts`, target branch `develop`, root stack predecessor `none`, root merge eligibility after review/CI, and expected PR title `Configuration - Phase 15: Raw Snapshot Opt-In And Source Artifact Hardening`. Public body mentions `@samcantrill` near the top and summarizes implementation scope, tests, validation, assumptions, risks, and GitHub checks without workflow internals.
- PR facts: opened https://github.com/samcantrill/loom/pull/42 with explicit base `develop`, head `codex/config-source-artifacts`, and title `Configuration - Phase 15: Raw Snapshot Opt-In And Source Artifact Hardening`. Immediate verification via `gh pr view 42 --json baseRefName,headRefName,state,url` returned `baseRefName=develop`, `headRefName=codex/config-source-artifacts`, `state=OPEN`, and the same PR URL. This is a root phase PR, not stacked on a predecessor branch, and is merge-eligible only after review/CI against `develop`; no retarget/rebase action is needed before review.
- PR scope confirmation: final diff is limited to Phase 15 config raw snapshot/source artifact hardening, the phase plan, and Phase 15 package/unit/contract/integration tests. No future Phase 16 documentation/e2e broadening, storage/run-store persistence, CLI behavior, pipeline integration, remote/plugin sources, or default raw-byte persistence was found in the changed files.
- Stack maintenance: root phase targeting `develop`; no stack predecessor or retarget/rebase action is needed before review.
