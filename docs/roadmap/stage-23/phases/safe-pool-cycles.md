...
- Public or durable shapes: the canonical disposition values are `started`,
  ...
  leases.
- Trust and failure boundaries: only coordination kind `capacity` can become
  queue deferral. Invalid/unsupported work terminates as failed under existing
  ownership wording. Unavailable/lost/internal coordination truth fails closed
  for the cycle and is not silently retried as queue policy.
...
- Private choices the executor may simplify: exact enum/class names if existing
  ...
  termination-state helpers, concrete UUID/token encoding, and compatibility
  constructor/normalizer. Identity non-reuse, dispositions, deadlines, and
  guarded semantics are fixed.
...
1. Add structured coordination failure kinds across the public error, SQLite,
   authority-service, and in-memory contract harness; make admission preserve
   those kinds and remove message-based retryability.
2. Introduce the three canonical dispatch dispositions and compatibility
   ...
   adapters.
3. Harden repository operations: atomic claim, expected-identity mutation
   guards, and audited claim deferral without changing DB schema v1.
4. Harden managed-local scalar lifecycle with session ownership, 50%/80%
   ...
   handle-commit compensation; aggregate the earliest maintenance timestamp.
...
| Unit | required | Dispositions, controller budgets, config, admission kinds, scalar renewal, and local compensation. | Reconcile/fill; replacement; defer; legacy `run_once()`; deadline/loss; exit-before-release. |
| Contract | required | Repository guards, result/evidence serialization, and coordination parity. | Round trips; no deferred handle; safe local keys only; stable kind across backends. |
| Integration | required | Real SQLite claim/defer/refill and managed-local scalar concurrency. | No duplicate/reused claim; stale writer conflicts; capacity queues; renewal/termination prevents overlap. |
...
- Main risks: non-atomic SQLite guards; reusable runtime identities; invalid
  disposition/field combinations; backend category drift; releasing a scalar
  lease before exit; and overstating a controller-local limit.
- Review focus: transactions/affected rows, identity non-reuse, dispositions,
  every stop condition, scalar renewal/termination ordering, and delegated and
  legacy one-step compatibility.
- Stop if: guards require queue DDL; a backend lacks a stable category; process
  exit cannot be confirmed through the runner seam; or compatibility requires
  changing locked behavior. Record evidence and return to the manager.
