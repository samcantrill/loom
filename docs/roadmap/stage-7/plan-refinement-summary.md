...
- Refined plan: `docs/roadmap/stage-7/implementation-plan.md`
- Source review report:
  `docs/roadmap/stage-7/plan-review-report.md`
...
| --- | --- | --- |
| Missing submitted stage-job startup semantics | Added the generic `SUBMITTED -> RUNNING` continuation contract, including identity/attempt/backend/submission metadata validation, Phase 1 scope, Phase 1 acceptance criteria, Phase 1 test obligations, and Phase 4 afterok startup acceptance. | `docs/roadmap/stage-7/implementation-plan.md` Key Design Choices, Phase 1, Phase 4 |
| Inconsistent `submission.json` vs `manifest.json` path | Standardized v7 on the existing v6 `slurm/submissions/<submission_id>/manifest.json` path and prohibited a second `submission.json` source of truth. | `docs/roadmap/stage-7/implementation-plan.md` Key Design Choices, Phase 2 |
| Underspecified submitted-operation registry | Added required generic registry fields, state values, active/terminal predicates, deterministic latest/latest-active ordering, backend metadata boundary, and shared submit/status/cancel predicate requirements. | `docs/roadmap/stage-7/implementation-plan.md` Key Design Choices, Extensibility Assessment, Phase 1 |
| Cancellation status outcomes not objective enough | Added a Phase 6 cancellation mutation matrix covering full, partial, unknown, already-terminal, and missing-command outcomes, plus acceptance and unit-test obligations for the matrix. | `docs/roadmap/stage-7/implementation-plan.md` Phase 6 |
