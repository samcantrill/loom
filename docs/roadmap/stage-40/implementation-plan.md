...
Current phase: 2 - agent-preparation (in_progress)
...
| 2 | agent-preparation | in_progress | [Durable preparation on shared storage](phases/agent-preparation.md) | agent/stage-40-p2-agent-preparation | pending | Common preparation lifecycle, shared capture/child/report, diagnostics/publisher, profile policy and root upgrade | Complete preparation and target execution using shared storage |
...
| 2 | Pending | Shared preparation lifecycle and native/CLI journeys implemented; focused worker placement, recovery, cancellation, report, retention and populated-root upgrade checks pass. Full gates and independent PR review remain outstanding; see the phase card | Loopback and fixture-root evidence; no physical deployment claim or real-root upgrade | Reuses the retained Stage 40 worktree; predecessor branches retired and synchronization passed |
