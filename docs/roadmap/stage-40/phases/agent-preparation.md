...
- Implementation: manager continuation integrates the complete shared preparation lifecycle, native child/report joins, bounded publication, cleanup retention and qualified exact-profile dispatch. Focused native/CLI, cancellation, retained-root upgrade and invalid-evidence checks pass; full gates remain required
...
- Blocker corrections: 1/3; full-gate process-spawn compatibility corrected with the existing copied-profile mapping representation
...
| Final focused lifecycle and CLI evidence | Working tree based on `462548f60e4085d14801d422bc17e8da646cdd69`: 2 cancellation/retained-upgrade cases passed in `/tmp/loom-stage40-p2-cancellation-retained-upgrade.log`; 2 Unix/HTTPS CLI journeys passed in `/tmp/loom-stage40-p2-preparation-cli.log`. A real blocking preparation child remains pending cancellation until native resource release, then retains terminal admission evidence. The upgrade fixture starts real completed and active admissions, preserves the old worker journal and coordinator identity, constructs the predecessor coordinator schema, invokes the actual offline CLI upgrade, and rejoins the retained worker to completion. This qualifies the predecessor storage shape with current native producers, not execution of an old binary or a real-root upgrade. CLI cases cover prepare/observe/guard/submit/cancel and staged refusal without reservation. |
| Final focused report and placement evidence | The extended HTTPS journey and 9 causal failure cases passed in `/tmp/loom-stage40-p2-final-boundaries.log` before its reserved-child error-detail failure. C receives no target assignment for an incompatible offered installation, then executes using its compatible profile. Real children cover failed/unavailable required checks, composition failure, nonportable paths, native-checksummed profile/requirement/coverage mismatches, mismatched committed worker result and corrupted report bytes. Publication is refused with the documented terminal failure code. The corrected reserved-child control error and remaining report/check cases all passed (7 cases; `/tmp/loom-stage40-p2-final-boundaries-corrected.log`). Public reserved-child submissions now report conflict/not_applied through a narrow pre-admission error; unrelated uncertain mutations keep their existing classification. |
| Full gate status | Initial test-typing diagnostics were corrected by narrowing generic native evidence in tests. The resumed `make validate-pr` at the validation-relevant tree committed as `84dbfa05dfbc6843e2cb43d53e49af9f64e0266a` passed Ruff and full Pyright, then baseline ran 3,244 passed, 5 failed, 2 skipped and 193 deselected (`/tmp/loom-stage40-p2-typed-tests-validate.log`). Four ordinary agent restart cases could not pickle the new root-mapping proxy; the fifth failure was the diagnostics export expectation. The smallest correction retains a validated copied dictionary, matching existing profile fields and leaving durable binding encoding unchanged, and includes `run_preflight_composed` in the expected public exports. All 24 selected existing restart/export/remote-boundary cases pass after correction, including a new populated-mapping process-serialization assertion (`/tmp/loom-stage40-p2-process-profile-regression.log`). Both full gates remain required. |
| PR, review, and merge | Pending; independent review and `make test-summary` have not run. The corrected `make validate-pr` remains required. |
| Residual risk and cleanup | Phase 2 remains incomplete until both full gates, independent actual-PR review, remote merge and synchronization pass. Physical fleet/NAS/Codex acceptance remains assigned to Phase 4; retained-root qualification uses isolated fixture roots. Persistent Stage 40 worktree retained; no real roots upgraded. |
