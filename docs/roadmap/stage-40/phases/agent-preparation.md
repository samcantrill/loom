...
- Implementation: manager owns continuation after the partial executor foundation; checked composition, bounded shared capture/access, protected profile selection/private mapping compatibility, offline upgrade and real resident-child/report checks pass; coordinator lifecycle and dispatch qualification remain in progress
...
| Item | Result |
| --- | --- |
| Implementation and changed paths | Foundation at `907fb929` adds native request/control and initial preparation rows. Checkpoint `15acc9609327138d8dd49de1c642d7fcd9d43934` records checked composition, finite shared capture/access, offline upgrade and product docs. Checkpoint `a91500fd902d6cd9aaba2f610c452fae4bd837a2` adds protected preparation policy, explicit existing-descriptor selection, effective shared-only aliases, active policy identity and optional retained worker root mappings. Manager continuation adds the fixed `loom.preparation.PreparationStage`, finite input binding through retained assignment/launch state, native preflight report decoding and plain-data publication adapter. Coordinator operation/lifecycle integration remains incomplete. |
...
| Residual risk and cleanup | Initial operation control still stops at acceptance/cancellation. Fixed child tests exercise a real resident subprocess, retained assignment/output state and direct native publication; they do not establish coordinator admission/committed-report joins or a complete prepare-to-execute journey. Actual preparation-module qualification/capability and exact-profile dispatch eligibility, durable selected-policy snapshot and target reservation, report joins, claim/recovery/cancellation/retention and actual native Unix/HTTPS journeys must be completed before Phase 2 acceptance. Upgrade storage tests do not replace the required live retained-admission/worker-journal integration case. Persistent Stage 40 worktree retained; no real roots upgraded. |
