...
- Phase execution plan path: `docs/roadmap/stage-9-post/phases/authority-inventory-contracts.md`
- Full plan: `docs/roadmap/stage-9-post/implementation-plan.md`
...
- Run and record an exhaustive inventory for `LocalRunStore`,
  ...
  `src`, `tests`, `examples`, `docs/features`, `docs/roadmap`, and `README.md`.
...
- The final inventory must be generated from these searches, run immediately
  ...
  `rg -n "LocalRunStore|LocalRunStorePaths" src tests examples docs/features docs/roadmap README.md`,
  `rg -n "\\bRunStore\\b" src tests examples docs/features docs/roadmap README.md`,
...
| --- | ---: | ---: | --- |
| `rg -n "LocalRunStore|LocalRunStorePaths" src tests examples docs/features docs/roadmap README.md` | 637 | 138 | Finds the concrete local store, path protocol, direct runtime construction, examples, tests, feature docs, and historical phase records. |
| `rg -n "\\bRunStore\\b" src tests examples docs/features docs/roadmap README.md` | 376 | 72 | Finds the current path-shaped aggregate protocol plus runtime/read annotations and docs that describe the overloaded name. |
| `rg -n "resolve_run_uri|allocate_run_uri|local_run_dir|local_stage_dir|local_artifact_root|local_stage_artifact_dir|local_config_path|local_provenance_path|local_stage_log_path|local_stage_worker_request_path|local_stage_worker_result_path|local_stage_workspace_dir|local_generated_artifact_path|local_run_freshness_path" src tests examples docs/features docs/roadmap README.md` | 374 | 55 | Finds local path helpers that must move to artifact/materialization roles or authority-backed factories in later phases. |
...
| `LRS-039` | `docs/roadmap/stage-9-post/implementation-plan.md`; `docs/roadmap/stage-9-post/planning.md` | Current plan guidance for `RunStore`, `StageStore`, `LocalRunStore` | docs/example | Active planning artifacts define this migration. | Keep as source-of-truth guidance and update status/metadata as phases complete. | All phases | Not a runtime escape hatch. |
| `LRS-040` | `docs/roadmap/stage-0/implementation-plan.md`; `docs/roadmap/stage-0-post/implementation-plan.md`; `docs/roadmap/stage-9/implementation-plan.md`; `docs/roadmap/stage-3/planning.md`; `docs/roadmap/stage-9/planning.md`; other roadmap notes from searches | Historical plan references | historical artifact | Older plans record the path-shaped local store history. | Do not rewrite unless a later doc-cleanup phase explicitly updates historical context. | none | Useful archaeology, not active behavior. |
| `LRS-041` | `docs/roadmap/stage-0-post/phases/v0-post-store-capabilities.md`; `add-local-stores-run-layout.md`; `add-local-execution.md`; `serial-write-integration.md`; `public-backend-swap.md`; `persistence-contracts.md`; `slurm-*`; `run-catalog-*`; `add-*`; corresponding PR bodies found by searches | Historical phase and PR artifacts | historical artifact | Past phase artifacts explain how the current local store became overloaded. | Preserve as historical records; future phases may cite them but should not mutate them for product behavior. | none | Current feature docs and code are the migration targets. |
| `LRS-042` | `docs/roadmap/stage-9-post/phases/authority-inventory-contracts.md` | This phase artifact self-references searches and classifications | docs/example | Current durable handoff and inventory. | Keep updated with completion notes and validation evidence. | Phase 1 | Self matches are expected after implementation. |
...
- Files and areas to inspect: `docs/roadmap/stage-9-post/phases/authority-inventory-contracts.md`;
  optional updates to `docs/roadmap/stage-9-post/implementation-plan.md`;
  ...
  `src/loom/runs`, `tests`, `examples`, `docs/features`, `docs/roadmap`, and `README.md`.
...
- PR preparation: draft body completed locally in
  `docs/roadmap/stage-9-post/phases/authority-inventory-contracts-pr-body.md`. Final validation
