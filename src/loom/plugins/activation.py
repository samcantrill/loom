...
class PluginActivationManifest:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != PLUGIN_ACTIVATION_MANIFEST_SCHEMA_VERSION:
            raise PluginInvalidEntryPointError(
                "PluginActivationManifest schema_version must be 1"
            )
        plugins = tuple(self.plugins)
        if any(not isinstance(record, PluginRecord) for record in plugins):
            raise PluginInvalidEntryPointError(
                "PluginActivationManifest plugins must be PluginRecord values"
            )
        if find_plugin_duplicates(plugins):
            raise PluginInvalidEntryPointError(
                "PluginActivationManifest plugins must have unique group/name records"
            )
        object.__setattr__(
            self,
            "plugins",
            tuple(
                sorted(plugins, key=lambda item: (item.group, item.name, item.value))
            ),
        )
    ...
    def from_dict(cls, data: object) -> "PluginActivationManifest":
        if not isinstance(data, Mapping):
            raise PluginInvalidEntryPointError(
                "PluginActivationManifest must be a mapping"
            )
        unknown = set(data) - {"schema_version", "plugins"}
        missing = {"schema_version", "plugins"} - set(data)
        if unknown or missing:
            raise PluginInvalidEntryPointError(
                "PluginActivationManifest must contain exactly schema_version and plugins"
            )
        version = data["schema_version"]
        records = data["plugins"]
        if isinstance(version, bool) or not isinstance(version, int):
            raise PluginInvalidEntryPointError(
                "PluginActivationManifest schema_version must be an integer"
            )
        if not isinstance(records, list):
            raise PluginInvalidEntryPointError(
                "PluginActivationManifest plugins must be a list"
            )
        return cls(
            schema_version=version,
            plugins=tuple(PluginRecord.from_summary(item) for item in records),
        )
...
def parse_plugin_selector(value: object) -> tuple[str, str]:
    ...
    if not isinstance(value, str) or value.count(":") != 1:
        raise PluginInvalidEntryPointError(
            "plugin selector must use exact GROUP:NAME syntax"
        )
...
def compare_plugin_activation_records(
    ...
    for key in sorted(set(expected) | set(actual)):
        ...
        if left is None:
            ...
            left.package is not None
            and right.package is not None
            and left.package != right.package
        ):
            findings.append(f"plugin package changed for {label}")
        elif (
            left.package_version is not None
            and right.package_version is not None
            and left.package_version != right.package_version
        ):
            findings.append(f"plugin package version changed for {label}")
        elif (
            left.package is None
            or right.package is None
            or left.package_version is None
            or right.package_version is None
        ):
...
__all__ = [
    "PLUGIN_ACTIVATION_MANIFEST_SCHEMA_VERSION",
