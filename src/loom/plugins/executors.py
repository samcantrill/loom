...
def load_executor_entry_points(
    ...
    *,
    ...
    return load_entry_points(
        _group_records(records),
        selected=_group_records(selected) if selected is not None else None,
        strict=strict,
        register=lambda record, value: _register(registry, record, value),
    )
...
def _group_records(records: Iterable[PluginRecord] | None) -> tuple[PluginRecord, ...]:
    return (
        ()
        if records is None
        else tuple(record for record in records if record.group == LOOM_EXECUTORS_GROUP)
    )
...
def _register(registry: ExecutorRegistry, record: PluginRecord, value: object) -> None:
    ...
    if registration.descriptor.name != record.name:
        raise TypeError(
            "executor entry-point name must match registration descriptor name"
        )
...
def _registration_from_value(value: object) -> ExecutorRegistration:
    ...
    if callable(value):
        ...
        if isinstance(candidate, ExecutorRegistration):
            return candidate
    raise TypeError(
        "executor entry point value must be an ExecutorRegistration or no-arg factory"
    )
