    ...
    *,
    ...
    for value, metadata in cases:
        ...
        if ready:
            ...
            encoded = encoded_result[1] if encoded_result[0] and isinstance(encoded_result[1], bytes) else None
            findings.append(_finding("codec.encode", encoded_result, require_type=bytes))
        else:
            findings.append(_failed("codec.encode", "codec protocol or key check failed; sample was not invoked"))
        if encoded is not None:
            ...
            if decoded_result[0]:
                findings.append(_finding("codec.roundtrip", _check(lambda: decoded_result[1] == value, "decoded value matches sample")))
            else:
                findings.append(_failed("codec.roundtrip", "codec decode failed; roundtrip comparison was not attempted"))
        else:
            findings.append(_failed("codec.decode", "codec encode failed; decode was not attempted"))
            findings.append(_failed("codec.roundtrip", "codec encode failed; roundtrip comparison was not attempted"))
...
def check_resource_validator_contract(
    ...
    *,
    ...
    kind_check = _check(lambda: bool(validate_resource_kind(kind)), "resource kind is valid")
    callable_check = _check(lambda: callable(validator), "resource validator is callable")
    registration_check = (
        _check(
        ...
        else (False, None, "resource kind or callable check failed; registration was not attempted")
    )
    findings = [
    ...
    for entry in valid:
        if ready:
            ...
            findings.append(_failed("resource_validator.accepts_valid", "validator prerequisites failed; entry was not invoked"))
        ...
        if ready:
            ...
            findings.append(
                ContractFinding(
                    ...
                    "validator rejected invalid entry" if not result[0] else "validator accepted invalid entry",
                )
            )
        else:
            findings.append(_failed("resource_validator.rejects_invalid", "validator prerequisites failed; entry was not invoked"))
...
def check_executor_contract(
    ...
    *,
    ...
    protocol = _check(lambda: isinstance(executor, Executor), "executor satisfies Executor")
    name = _check(
        lambda: isinstance(getattr(executor, "name"), str) and bool(getattr(executor, "name")),
        "executor name is a non-empty string",
    )
    findings = [_finding("executor.protocol", protocol), _finding("executor.name", name)]
    ...
    for request in request_values:
        if not ready:
            findings.extend(
                (
                    _failed("executor.execute", "executor protocol or name check failed; request was not invoked"),
                    _failed("executor.result_type", "executor request was not invoked"),
                    _failed("executor.result_identity", "executor request was not invoked"),
                )
            )
            continue
        execution = _check(
        ...
        findings.append(_finding("executor.result_type", execution, require_type=StageExecutionResult))
        if execution[0] and isinstance(execution[1], StageExecutionResult):
            ...
            findings.append(_failed("executor.result_identity", "executor did not return StageExecutionResult"))
...
def _check(operation: Callable[[], Any], message: str) -> tuple[bool, object | None, str]:
    try:
        result = operation()
    except Exception as exc:  # Trusted caller samples; reports retain bounded exception facts.
        return False, None, f"{message}: {type(exc).__name__}: {str(exc) or type(exc).__name__}"
    if result is False:
...
def _finding(
    ...
    *,
    ...
    if passed and require_type is not None and not isinstance(value, require_type):
        return ContractFinding(code, "fail", f"{message}: returned {type(value).__name__}, expected {require_type.__name__}")
