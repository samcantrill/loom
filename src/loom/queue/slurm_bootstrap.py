...
def _read_job_private_capability(path: Path) -> bytes:
    try:
        descriptor = os.lstat(path)
        if not stat.S_ISREG(descriptor.st_mode) or stat.S_ISLNK(descriptor.st_mode):
            raise QueueServiceError("SLURM capability file is not a regular file")
        if descriptor.st_size < 16 or descriptor.st_size > 4096:
            raise QueueServiceError("SLURM capability file size is invalid")
        fd = os.open(path, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0))
        try:
            opened = os.fstat(fd)
            if not stat.S_ISREG(opened.st_mode) or opened.st_ino != descriptor.st_ino:
                raise QueueServiceError("SLURM capability file changed during read")
            value = os.read(fd, 4097)
        finally:
            os.close(fd)
    except OSError as exc:
        raise QueueServiceError("SLURM job-private capability is unavailable") from exc
    if len(value) != descriptor.st_size:
        raise QueueServiceError("SLURM capability file is invalid")
    return value


def _unlink_job_private_capability(path: Path) -> None:
    try:
        path.unlink()
    except OSError as exc:
        raise QueueServiceError("SLURM job-private capability unlink failed") from exc
