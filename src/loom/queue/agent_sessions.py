...
class AgentSessionService:
    ...
    def register(self, request: AgentRegistration) -> AgentSession:
        ...
        with self._daemon._connection() as conn:  # type: ignore[attr-defined]
            ...
            conn.execute(
                "INSERT INTO agent_sessions(session_id, agent_id, agent_root_id, principal_id, policy_revision, config_revision, inventory_revision, availability_revision, capabilities_json, pools_json, coordinator_epoch, state, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    session.session_id,
                    rule.agent_id,
    ...
    def wait_for_work(
        ...
        *,
        ...
        try:
            ...
            with self._daemon._connection() as conn:  # type: ignore[attr-defined]
                conn.execute(
                    "UPDATE agent_polls SET active = 0 WHERE poll_id = ? AND digest = ?",
                    (poll_id, digest),
                )
                conn.commit()
            raise
    ...
    def retire_clean(
        ...
        # First durably withdraw the offer, fence the poll, and prevent new
        # session references. A later proof failure remains safely withdrawn.
        with self._daemon._connection() as conn:  # type: ignore[attr-defined]
            conn.execute("BEGIN IMMEDIATE")
            replay = _receipt(
        ...
        with self._daemon._connection() as conn:  # type: ignore[attr-defined]
            ...
            conn.execute(
                ...
                (proof.session_id, _canonical_json(proof.value())),
            )
            conn.execute(
                "UPDATE agent_sessions SET state = ? WHERE session_id = ?",
...
def initialize_agent_session_schema(
    ...
    if coordinator:
        ...
        CREATE TABLE IF NOT EXISTS agent_polls (poll_id TEXT PRIMARY KEY, principal_id TEXT NOT NULL, session_id TEXT NOT NULL, availability_revision TEXT NOT NULL, coordinator_epoch TEXT NOT NULL, wait_timeout_ms INTEGER NOT NULL, digest TEXT NOT NULL, active INTEGER NOT NULL, result_json TEXT);
        ...
        conn.executescript("""
        CREATE TABLE IF NOT EXISTS agent_registration_intents (operation_id TEXT PRIMARY KEY, digest TEXT NOT NULL, request_json TEXT NOT NULL, result_json TEXT);
        CREATE TABLE IF NOT EXISTS agent_sessions_local (session_id TEXT PRIMARY KEY, value_json TEXT NOT NULL, state TEXT NOT NULL);
...
def validate_agent_session_schema(
    ...
    """Reject a current-version root whose required Phase 4 tables are missing."""
    required = (
        {
            "agent_sessions": {
                "session_id",
        ...
        if coordinator
            ...
            "agent_sessions_local": {"session_id", "value_json", "state"},
