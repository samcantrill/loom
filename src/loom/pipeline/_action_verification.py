...
def action_candidate(value: object) -> dict[str, PlainData]:
    ...
    if candidate["candidate_digest"] != hash_mapping(
    ...
    if set(producer) != {
        ...
        "assignment_id",
        "fencing_token",
    } or any(not isinstance(item, str) or not item for item in producer.values()):
        raise ValueError("invalid original action producer")
    result = plain_mapping(candidate["result"])
    if set(result) != {"commit", "artifact_facts"} or not isinstance(
        result["artifact_facts"], list
    ):
        raise ValueError("invalid action result facts")
    commit = OutputCommitRecord.from_dict(result["commit"])
    facts = tuple(
        ArtifactFactRecord.from_dict(item) for item in result["artifact_facts"]
    )
    if (producer["run_uri"], producer["node_id"], producer["attempt_id"]) != (
        commit.run_uri,
        commit.stage_name,
        commit.attempt_id,
    ):
    ...
    if (
        set(outputs) != set(commit.output_names)
        or {fact.artifact_name for fact in facts} != set(outputs)
        or len(facts) != len(outputs)
        or any(fact.commit_id != commit.commit_id for fact in facts)
    ):
...
def action_verification_request(value: object) -> dict[str, PlainData]:
    ...
    if (
        set(request) != {"node_id", "contract", "candidate"}
    ...
    if (
        contract["semantic_key"] is None
        or contract["semantic_key"] != candidate["semantic_key"]
        or contract["namespace"] != candidate["namespace"]
    ):
...
def action_verification_response(
    value: object, candidate: Mapping[str, PlainData]
) -> dict[str, PlainData]:
    ...
    if (
        ...
        or response.get("candidate_digest") != candidate["candidate_digest"]
    ):
    ...
    if (
        set(reason) != {"code", "output_port"}
        or not isinstance(reason["code"], str)
        or re.fullmatch(r"[a-z][a-z0-9_]{0,63}", reason["code"], flags=re.ASCII) is None
    ):
        raise ValueError("invalid action verification reason")
    port = reason["output_port"]
    if port is not None and (
        not isinstance(port, str)
        or port not in cast(Mapping[str, PlainData], candidate["output_contracts"])
    ):
