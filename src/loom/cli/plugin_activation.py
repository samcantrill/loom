...
from loom.pipeline.resources import DEFAULT_RESOURCE_VALIDATOR_REGISTRY, ResourceValidatorRegistry
from loom.plugins import (
    PluginRecord, list_entry_points, load_codec_entry_points, load_executor_entry_points,
...
def add_plugin_option(parser: object) -> None:
    """Attach the shared explicit repeatable selector option to a CLI parser."""
    parser.add_argument("--plugin", action="append", default=None, metavar="GROUP:NAME", help="explicit runtime plugin; may be repeated")


def selected_runtime_plugins(
    selectors: Iterable[str] | None, *, allowed_groups: Iterable[str],
) -> tuple[PluginRecord, ...]:
    values = tuple(selectors or ())
    if not values:
        return ()
    return resolve_plugin_selections(
        values, list_entry_points(groups=tuple(allowed_groups)), allowed_groups=allowed_groups
    )


def build_selected_registries(
    records: Iterable[PluginRecord], *, base_codecs: CodecRegistry | None = None,
    base_validators: ResourceValidatorRegistry = DEFAULT_RESOURCE_VALIDATOR_REGISTRY,
    executor_registry: ExecutorRegistry | None = None,
) -> tuple[CodecRegistry, ResourceValidatorRegistry, ExecutorRegistry, PluginActivationManifest]:
    ...
    executors = ExecutorRegistry() if executor_registry is None else executor_registry
    load_codec_entry_points(selected, codecs, selected=selected, strict=True)
    validators, _ = load_resource_validator_entry_points(selected, base_validators, selected=selected, strict=True)
...
__all__ = ["add_plugin_option", "build_selected_registries", "selected_runtime_plugins"]
